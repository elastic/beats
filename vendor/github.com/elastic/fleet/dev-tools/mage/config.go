...
var (
	shortTemplate     = filepath.Join("build", "agent.yml.tmpl")
	referenceTemplate = filepath.Join("build", "agent.reference.yml.tmpl")
	dockerTemplate    = filepath.Join("build", "agent.docker.yml.tmpl")
)
...
func Config(types ConfigFileType, args ConfigFileParams, targetDir string) error {
	...
	// Short
	if types.IsShort() {
		file := filepath.Join(targetDir, "agent.yml")
	...
	if types.IsReference() {
		file := filepath.Join(targetDir, "agent.reference.yml")
	...
	// Docker
	if types.IsDocker() {
		file := filepath.Join(targetDir, "agent.docker.yml")
