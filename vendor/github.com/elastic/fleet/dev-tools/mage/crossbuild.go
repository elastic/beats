...
func (b GolangCrossBuilder) Build() error {
	...
	//buildCmd, err := filepath.Rel(workDir, filepath.Join(mountPoint, "go build -o agent -i "+filepath.Join(mountPoint, "cmd", "agent", "agent.go")))
	...
	if usr, err := user.Current(); err == nil {
		cacheDir := filepath.Join(usr.HomeDir, "go", "pkg", "mod")
		cacheMountPoint := "/go/pkg/mod"

		args = append(args,
			"--rm",
			"--env", "MAGEFILE_VERBOSE="+verbose,
			"--env", "MAGEFILE_TIMEOUT="+EnvOr("MAGEFILE_TIMEOUT", "5m"),
			"--env", "GO111MODULE=on",
			"-v", repoInfo.RootDir+":"+mountPoint,
			"-v", cacheDir+":"+cacheMountPoint+":ro",
			"-w", workDir,
			image,
		...
		args = append(args,
			"--rm",
			"--env", "MAGEFILE_VERBOSE="+verbose,
			"--env", "MAGEFILE_TIMEOUT="+EnvOr("MAGEFILE_TIMEOUT", "15m"),
			"--env", "GO111MODULE=on",
			"-v", repoInfo.RootDir+":"+mountPoint,
			"-w", workDir,
			image,
