...
const (
	...
	elasticBeatsImportPath = "github.com/elastic/fleet"
)
...
var (
	...
	BeatName        = EnvOr("BEAT_NAME", defaultBeatName)
...
var (
	beatVersionRegex = regexp.MustCompile(`(?m)^var version = "(.+)"\r?$`)
...
var (
	...
	DefaultBeatBuildVariableSources = &BuildVariableSources{
		BeatVersion: "{{ elastic_beats_dir }}/pkg/release/version.go",
...
func getProjectRepoInfo() (*ProjectRepoInfo, error) {
	...
	repoInfo, err := getRepoInfoFromGopath(cwd)
	if err == nil {
		return repoInfo, nil
	}
	...
	repoInfo, err = getRepoInfoFromIncludePath(cwd)
	if err == nil {
		return repoInfo, nil
	}
	...
	repoInfo, err = getRepoInfoByTraversal(cwd)
	if err == nil {
		return repoInfo, nil
	}
...
func getRepoInfoFromGopath(cwd string) (*ProjectRepoInfo, error) {
	var errs []string
	var srcDir, rootImportPath string
...
func getRepoInfoFromIncludePath(cwd string) (*ProjectRepoInfo, error) {
	var errs []string
	var srcDir, rootImportPath string

	for _, path := range filepath.SplitList(cwd) {
		path = filepath.Clean(path)

		fleetIdx := strings.Index(path, elasticBeatsImportPath)
		if fleetIdx < 0 {
			continue
		}
		srcDir = path[:fleetIdx]
		_, root, err := vcs.FromDir(cwd, srcDir)
		if err != nil {
			// Try the next path.
			errs = append(errs, err.Error())
...
func getRepoInfoByTraversal(cwd string) (*ProjectRepoInfo, error) {
	var errs []string
	var srcDir string

	// TODO: this more single purpose approach, the problem is getting a remote (import path)
	// from a repo when being on path such as `/go/code`
	paths := filepath.SplitList(cwd)
	...
	for _, path := range paths {
		...
		for {
			...
			_, _, err := vcs.FromDir(path, srcDir)
			if err != nil {
				path = srcDir
				// Try the next path.
				errs = append(errs, err.Error())
			...
			return &ProjectRepoInfo{
				RootImportPath: elasticBeatsImportPath,
				RootDir:        path,
				SubDir:         filepath.Join("dev-tools", "mage"),
				ImportPath:     strings.Join([]string{elasticBeatsImportPath, "dev-tools/mage"}, "/"),
			}, nil
		}
	}
