...
GUI_APP=Finder
IDENTIFIER="co.elastic.beats.agent"
...
if [ "$(id -u)" -ne 0 ]; then
    ...
    osascript -- - "$cmd" <<EOF
on run argv
    do shell script item 1 of argv with administrator privileges with prompt "Agent uninstaller"
end run
EOF
    exit 0
fi
...
pkgutil --pkg-info "$IDENTIFIER" > /dev/null || die "agent not installed"
...
BEAT_DIR="/Library/Application Support/Elastic/agent"
if [ -d "$BEAT_DIR" -a -n "agent" -a -n "Elastic" ]; then
    rm -rf "$BEAT_DIR"
fi

dialog note 'Agent successfully uninstalled'
