...
// - process id
// - error
func Start(logger *ecslog.Logger, path string, config *Config, uid, gid int, creds *Creds, arg ...string) (processInfo *Info, err error) {
	// inject env
...
func Stop(logger *ecslog.Logger, pid int) error {
...
// itself for awaiter
func Attach(logger *ecslog.Logger, pid int) (*Info, error) {
	proc, err := os.FindProcess(pid)
	if err != nil {
