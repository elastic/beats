...
[discrete]
[[new-features-8.19.11]]
==== New features

**Osquerybeat**

* Integrate macadmins/osquery-extension tables into osquerybeat extension. Adds cross-platform tables (puppet_info, puppet_logs, puppet_state, puppet_facts, google_chrome_profiles, file_lines), Linux/macOS tables (crowdstrike_falcon), and macOS-specific tables (energy_impact, filevault_users, local_network_permissions, macos_profiles, mdm, munki_info, munki_installs, pending_apple_updates, macadmins_unified_log, macos_rsr, authdb, wifi_network, alt_system_info). Note: sofa_* and network_quality tables are excluded due to external network dependencies.
