...
- Fix templates being overwritten if there was an error when check for the template existence. {pull}24332[24332]
...
- Allow embedding of CAs, Certificate of private keys for anything that support TLS in outputs and inputs. {pull}21179[21179]
...
- Fix failures caused by custom beat names with more than 15 characters {pull}22550[22550]
...
- Change `vsphere.datastore.capacity.used.pct` value to between 0 and 1. {pull}23148[23148]
...
- Include log.source.address for unparsable syslog messages. {issue}13268[13268] {pull}15453[15453]
...
- Remove `in_cluster` configuration parameter for Kubernetes, now in-cluster configuration is used only if no other kubeconfig is specified {pull}13051[13051]
...
- Add a check so alias creation explicitly fails if there is an index with the same name. {pull}13070[13070]
...
- Update the code of Central Management to align with the new returned format. {pull}10019[10019]
- Allow Central Management to send events back to kibana. {issue}9382[9382]
- Fix panic if fields setting is used to configure `hosts.x` fields. {issue}10824[10824] {pull}10935[10935]
...
- Package: Disable librpm signal handlers. {pull}10694[10694]
- Login: Handle different bad login UTMP types. {pull}10865[10865]
- Fix hostname references in System module dashboards. {pull}11064[11064]
- User dataset: Numerous fixes to error handling. {pull}10942[10942]
...
- Fix size of maximum mapped read area in spool file on Windows. {pull}7859[7859]
...
- watch.poll_file is now deprecated and superseded by automatic config file reloading.
...
- Fix a type issue when specifying certificate authority when using the `import_dashboards` command. {pull}6678[6678]
