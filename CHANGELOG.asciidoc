...
== Beats version 8.19.22

https://github.com/elastic/beats/compare/v8.19.21\...v8.19.22[View commits]
...
* Add `api_endpoint` configuration option to the `gcp-pubsub` input to allow overriding the default Pub/Sub API endpoint. https://github.com/elastic/beats/pull/53080[#53080]
...
== Beats version 8.19.21

https://github.com/elastic/beats/compare/v8.19.20\...v8.19.21[View commits]
...
https://github.com/elastic/beats/compare/v8.19.19\...v8.19.20[View commits]
...
// tag::known-issue-8.19-disk-queue-filled-metrics[]
...
// end::known-issue-8.19-disk-queue-filled-metrics[]

**Filebeat**

include::CHANGELOG.asciidoc[tags=known-issue-8.19-filestream-take-over]
...
include::CHANGELOG.asciidoc[tags=known-issue-8.19-disk-queue-filled-metrics]
...
https://github.com/elastic/beats/compare/v8.19.17\...v8.19.18[View commits]
...
https://github.com/elastic/beats/compare/v8.19.16\...v8.19.17[View commits]
...
* Upgrade Go to v1.26.4. https://github.com/elastic/beats/pull/51163[#51163]
...
* Deprecate `ssl.restart_on_cert_change` in favor of `ssl.certificate_reload`. https://github.com/elastic/beats/pull/51022[#51022]
...
* Update Go to v1.26.3. https://github.com/elastic/beats/pull/50843[#50843]

**Beats**

* Allow TLS certificate and CA hot-reload without restarting. https://github.com/elastic/beats/pull/51022[#51022]
...
* Migrate from the deprecated `github.com/docker/docker` module to the `github.com/moby/moby` split modules. https://github.com/elastic/beats/pull/50300[#50300]

**Filebeat**

* Add input redirection support through a new Redirector mechanism. https://github.com/elastic/beats/pull/51310[#51310]

**Metricbeat**

* Migrate `azure/app_insights` metricset off the deprecated track-1 Azure SDK and `go-autorest`, using `azcore` directly. https://github.com/elastic/beats/pull/50648[#50648]
...
* Fix panic in the Elastic Agent V2 manager when reloading with no output unit. https://github.com/elastic/beats/pull/51260[#51260]

**Filebeat**

* Fix token refresh for `jwk_pem/jwk_file` in the `cel` and `httpjson` inputs. https://github.com/elastic/beats/pull/50826[#50826] https://github.com/elastic/beats/issues/50426[#50426]
* Respect `max_bytes`/`message_max_bytes` when reading the first chunk of CRI partial lines. https://github.com/elastic/beats/pull/50796[#50796]
* Fix request tracer path validation for the `cel`, `httpjson`, `http_endpoint`, and `entityanalytics` inputs when Filebeat runs as an OTel receiver. https://github.com/elastic/beats/pull/51026[#51026]
* Make CrowdStrike streaming input cancel refresh goroutines on session reconnect. https://github.com/elastic/beats/pull/50864[#50864]
...
* Cache Okta OAuth2 token in `cel` and `httpjson` to avoid unnecessary JWT regeneration. https://github.com/elastic/beats/pull/51177[#51177]
* Fix filestream registry leak on file renames. https://github.com/elastic/beats/pull/51290[#51290]
* Guard `event.original` rename in Azure module ingest pipelines to prevent a "field already exists" error when the field is pre-populated. https://github.com/elastic/beats/pull/51271[#51271]
...
* Fix a data race in `add_cloud_metadata` processor when fetching metadata from multiple providers concurrently. https://github.com/elastic/beats/pull/51317[#51317]

**Osquerybeat**

* Respect Osquery pack query platform filters for Live Query actions. https://github.com/elastic/beats/pull/51118[#51118]
...
* Treat RPC_S_UNKNOWN_IF (1717) as a recoverable error so Winlogbeat reopens the event log session instead of exiting on this transient RPC error. https://github.com/elastic/beats/pull/51031[#51031]
...
https://github.com/elastic/beats/compare/v8.19.15\...v8.19.16[View commits]
...
* When Filestream is migrating the registry key from inputs that did not have an ID, match files by path and file identity instead of only path. https://github.com/elastic/beats/pull/50731[#50731]
...
https://github.com/elastic/beats/compare/v8.19.13\...v8.19.14[View commits]
...
// tag::known-issue-8.19-filestream-take-over[]
- Filestream "take over" mode does not work in versions 8.19.0-8.19.20. A fix will ship in the 8.19.21 release.
// end::known-issue-8.19-filestream-take-over[]
// tag::known-issue-8.19-filestream-clean-inactive-1[]
...
// end::known-issue-8.19-filestream-clean-inactive-1[]
...
// tag::known-issue-8.19-journalctl[]
...
====
// end::known-issue-8.19-journalctl[]

==== Breaking changes

*Affecting all Beats*

- The base folder for data and logs on Windows has changed from `C:\ProgramData\[BeatName]` to to `C:\Program Files\[BeatName]-Data` because the latter has stricter permissions. {pull}45948[45948]
...
- Update Go version to 1.24.5. {pull}45403[45403]
...
include::CHANGELOG.asciidoc[tags=known-issue-8.19-filestream-take-over]
include::CHANGELOG.asciidoc[tags=known-issue-8.19-filestream-clean-inactive-1]
include::CHANGELOG.asciidoc[tags=known-issue-8.19-restart_on_cert_change]
include::CHANGELOG.asciidoc[tags=known-issue-8.19-journalctl]
...
- Use Debian 11 to build linux/arm to match linux/amd64. Upgrades linux/arm64's statically linked glibc version from 2.28 to 2.31. {issue}44816[44816]

*Filebeat*

- Journald input now works on Docker containers, all image variants except Wolfi. {issue}41278[41278] {issue}44040[44040] {pull}44295[44295]
...
- Upgrade `go.mongodb.org/mongo-driver` from version `v1.14.0` to `v1.17.4` to fix connection leaks in MongoDB module. {pull}44769[44769]
...
- Update Go version to 1.24.4. {pull}44696[44696]
...
- Fix the 'add_cloud_metadata' processor to better support custom certificate bundles by improving how the AWS provider HTTP client is overridden. {pull}44189[44189]

*Auditbeat*

- Fix a potential error in the system/package component that could occur during internal package database schema migration. {issue}44294[44294] {pull}44296[44296]
...
- Disable the allow_unsafe osquery configuration. {pull}40130[40130]

==== Added

*Affecting all Beats*

- Update Go version to v1.24.3. {pull}44270[44270]

*Metricbeat*

- Add checks for the Resty response object in all Meraki module API calls to ensure proper handling of nil responses. {pull}44193[44193]
- Add `enable_batch_api` option in the Azure monitor module to allow metrics collection of multiple resources using Azure batch API. {pull}41790[41790]
- Add support for `_nodes/stats` URIs compatible with legacy Elasticsearch versions. {pull}44307[44307]
- Add a latency configuration option to the Azure Monitor module. {pull}44366[44366]

*Osquerybeat*

- Update osquery version to v5.15.0. {pull}43426[43426]
...
- Journald input now works on Docker containers (only ubi variant). {issue}41278[41278] {issue}44040[44040] {pull}44056[44056]
...
- Add a cached hasher for upcoming backend. {pull}41952[41952]
...
- Update CEL mito extensions version to v1.16.0. {pull}41727[41727]
...
- Upgrade node version to latest LTS v18.20.7. {pull}43511[43511]
...
https://github.com/elastic/beats/compare/v8.17.9\...v8.17.10[View commits]
...
- Upgrade `go.mongodb.org/mongo-driver` version from `v1.14.0` to `v1.17.4` to fix connection leaks in MongoDB module. {pull}44769[44769]
- Fixed a bug where `event.duration` could be missing from an event on Windows systems due to low-resolution clock. {pull}44440[44440]
...
=== Beats version 8.17.7
https://github.com/elastic/beats/compare/v8.17.6\...v8.17.7[View commits]
...
https://github.com/elastic/beats/compare/v8.17.5\...v8.17.6[View commits]
...
- Journald input is again usable in the Filebeat Docker container (except Wolfi) by including journalctl by default. The included version of journalctl must be compatible with the one on that wrote the journal files being ingested. The Wolfi container remains distroless and does not contain journalctl or any part of systemd. {issue}41278[41278] {issue}44040[44040] {pull}44056[44056]
- Fixed websocket input panic on sudden network error or server crash. {issue}44063[44063] {pull}44068[44068]
...
https://github.com/elastic/beats/compare/v8.17.4\...v8.17.5[View commits]
...
- Handle permission errors while collecting data from Windows services and don't interrupt the overall collection by skipping affected service. {issue}40765[40765] {pull}43665[43665]
...
https://github.com/elastic/beats/compare/v8.17.3\...v8.17.4[View commits]
...
- Prevent computer details being returned for user queries by Active Directory Entity Analytics provider. {issue}11818[11818] {pull}42796[42796]
- Handle unexpectedEOF error in `aws-s3` input and enforce retrying using download failed error. {pull}42420[42420]
...
- Update Go version to 1.23.6. {pull}42705[42705]
...
// tag::known-issue-8.19-filestream-clean-inactive-long-value[]
- The Filestream input does not enforce the restrictions documented for the `clean_inactive` option, thus allowing configurations that can lead to data re-ingestion issues.
...
// end::known-issue-8.19-filestream-clean-inactive-long-value[]
...
include::CHANGELOG.asciidoc[tags=known-issue-8.19-filestream-clean-inactive-long-value]
