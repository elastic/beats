...
- Fix `NewContainerMetadataEnricher` to use default config for kubernetes module. No longer requires the user to have `labels.dedot: true` in the configuration as it is now properly default. {pull}16857[16857]
...
- Added `access_key_id`, `secret_access_key`, and `session_token` to the aws module config. {pull}17456[17456]
...
- Make the azure module GA. {pull}17319[17319]
- In the kubernetes manifests, mount the data directory from the host, so data persist between executions in the same node. {pull}17429[17429]
