...
- Add fields for mermory fragmentation, memory allocator stats, copy on write, master-slave status, and active defragmentation to `info` metricset of Redis module.
...
- Redis `info` `replication.master_offset` has been deprecated in favor of `replication.master.offset`.
