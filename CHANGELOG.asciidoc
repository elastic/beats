...
=== Beats version avoid-hardcoded-slash-log-in-docker-path
https://github.com/jp-paillet/beats.git[Check the HEAD diff]
...
- Allow beats to parse docker inputs outside an hardcoded directory structure. {pull}9620[9620]
...
- Reworked the initialization of container's paths. {pull}9620[9620]
...
- Added `containers.suffix` input configuration option to avoid the static addition of the "/*.log" in paths. {issue}8988[8988] {pull}9620[9620]
