...
- `restart_on_cert_change` causes panic due to seccomp policy. In versions 8.17.3 and later, enabling this option causes the Beat to panic on restart. This is due to the `eventfd2` syscall missing from the default seccomp policy. To fix this, add `eventfd2` to a custom seccomp policy. To fix this, add `eventfd2` to a custom seccomp policy. {filebeat-ref}/linux-seccomp.html[Use Linux Secure Computing Mode (seccomp)].
