...
[ -z "${run_heartbeat_arm_tests+x}" ] && run_heartbeat_arm_tests="$(buildkite-agent meta-data get run_heartbeat_arm_tests --default "false")"
...
[ -z "${run_heartbeat_tests+x}" ] && run_heartbeat_macos_tests="$(buildkite-agent meta-data get run_heartbeat_macos_tests --default "false")"
...
[ -z "${run_heartbeat_win_tests+x}" ] && run_heartbeat_win_tests="$(buildkite-agent meta-data get run_heartbeat_win_tests --default "false")"
...
case "${BUILDKITE_PIPELINE_SLUG}" in
  ...
  "filebeat")
    ...
    ;;
  "heartbeat")
    BEAT_CHANGESET_REFERENCE=${heartbeat_changeset[@]}
    ;;  
  "beats-metricbeat")
...
are_conditions_met_macos_tests() {
  if are_conditions_met_mandatory_tests; then    #from https://github.com/elastic/beats/blob/c5e79a25d05d5bdfa9da4d187fe89523faa42afc/Jenkinsfile#L145-L171
    if [[ "$BUILDKITE_PIPELINE_SLUG" == "auditbeat" || "$BUILDKITE_PIPELINE_SLUG" == "filebeat" || "$BUILDKITE_PIPELINE_SLUG" == "heartbeat" || "$BUILDKITE_PIPELINE_SLUG" == "beats-metricbeat" || "$BUILDKITE_PIPELINE_SLUG" == "beats-packetbeat" || "$BUILDKITE_PIPELINE_SLUG" == "beats-xpack-metricbeat" || "$BUILDKITE_PIPELINE_SLUG" == "beats-xpack-auditbeat" || "$BUILDKITE_PIPELINE_SLUG" == "beats-xpack-filebeat" || "$BUILDKITE_PIPELINE_SLUG" == "beats-xpack-heartbeat" || "$BUILDKITE_PIPELINE_SLUG" == "beats-xpack-osquerybeat" ]]; then
...
are_conditions_met_win_tests() {
  if are_conditions_met_mandatory_tests; then    
    if [[ "$BUILDKITE_PIPELINE_SLUG" == "auditbeat" || "$BUILDKITE_PIPELINE_SLUG" == "filebeat" || "$BUILDKITE_PIPELINE_SLUG" == "heartbeat" ]]; then
