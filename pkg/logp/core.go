...
func ConfigureWithTypedOutputLocal(defaultLoggerCfg, typedLoggerCfg Config, key, value string, outputs ...zapcore.Core) (*Logger, error) {
	root, selectors, level, err := buildTypedOutputRoot(defaultLoggerCfg, typedLoggerCfg, key, value, outputs...)
	if err != nil {
		return nil, err
	}
...
func ConfigureWithTypedOutputNonGlobal(defaultLoggerCfg, typedLoggerCfg Config, key, value string, outputs ...zapcore.Core) (*Logger, error) {
	root, selectors, _, err := buildTypedOutputRoot(defaultLoggerCfg, typedLoggerCfg, key, value, outputs...)
	if err != nil {
		return nil, err
	}
	return newLogger(root, selectors), nil
...
// buildTypedOutputRoot assembles the zap root logger and selector list for a logger
// that routes matching entriesto a separate typed output.
func buildTypedOutputRoot(defaultLoggerCfg, typedLoggerCfg Config, key, value string, outputs ...zapcore.Core) (*zap.Logger, map[string]struct{}, zap.AtomicLevel, error) {
	sink, level, selectors, err := createSink(defaultLoggerCfg, outputs...)
	if err != nil {
	...
	typedCore, err := createLogOutput(typedLoggerCfg, level)
	if err != nil {
		return nil, nil, zap.AtomicLevel{}, fmt.Errorf("could not create typed logger output: %w", err)
	}
	...
	root := zap.New(sink, makeOptions(defaultLoggerCfg)...)
	return root, selectors, level, nil
}
