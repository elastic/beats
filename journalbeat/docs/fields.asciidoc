...
////
This file will get updated with the fields from your Beat when you run
make update.
////
...
--
This document describes the fields that are exported by Beatname. They are
...
The input type from which the event was generated. This field is set to the value specified for the `type` option in the input section of the Beatname config file. (DEPRECATED: see `input.type`)
...
The input type from which the event was generated. This field is set to the value specified for the `type` option in the input section of the Beatname config file.
...
+
--
The Beatname module that generated this event.
...
+
--
The Beatname fileset that generated this event.
...
event.created contains the date on which the event was created. In case of log events this is when the log line was read by Beatname. In comparison @timestamp is the processed timestamp from the log line. If both are identical only @timestamp should be used.
