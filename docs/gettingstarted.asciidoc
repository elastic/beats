...
The simplest Topbeat setup includes Topbeat, Elasticsearch, Kibana and optionally Logstash. To get started follow the
following steps:

 * Install Topbeat on your servers to capture system wide and per process statistics
 * TODO[Install Elasticsearch] for storing the data.
 * TODO[Install Kibana] for visualizing the statistics from your servers

More details about the sample Kibana dashboards are available TODO[here].
