...
Note: The minimum required Logstash version for this plugin is 1.5.4.
...
the required plugin run the following command inside the logstash directory. In
case you used deb or rpm for installation, run the logstash plugin executable
instead.
...
TLS must be explicitly enabled in the Beat and Logstash configuration.
