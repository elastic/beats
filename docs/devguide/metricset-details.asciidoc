...
You can find up to 3 `fields.yml` files in the beats repository for each metricbeat module:

[float]
===== metricbeat/fields.yml

This file is located at `metricbeat/fields.yml` and it is used for different purposes:
...
Here is an example for the `fields.yml` file from the MySQL module.
...
[float]
===== metricbeat/[module]/[metricset]/_meta/fields.yml

The last `fields.yml` file is under `metricbeat/module/{module}/{metricset}/_meta/fields.yml` and it contains all fields retrieved
