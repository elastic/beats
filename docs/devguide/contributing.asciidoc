...
NOTE: One deterministic manner to install Go in the proper version to work with Beats is using
https://github.com/andrewkroh/gvm[GVM] Go version manager. An example for Mac users would be:

[source,shell]
----------------------------------------------------------------------
gvm use {go-version}
export GOROOT="/Users/${USER}/.gvm/versions/go{go-version}.darwin.amd64"
export PATH="/Users/${USER}/.gvm/versions/go{go-version}.darwin.amd64/bin:$PATH"
----------------------------------------------------------------------
...
Then you can compile a particular Beat by using mage commands. For example, for
Packetbeat:
...
the available make targets with `mage`.
