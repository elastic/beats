...
First we have to build the docker image which is available for the modules. As an example let's take MySQL module.

This steps assume you have checked out the Beats repository from Github and are inside `beats` directory:
...
This changes your directory to where the Dockerfile is for MySQL and builds the docker image. In the end you should see something like:
...
The last has after `Successfully built` is the important one. It's the one we run our container on. Before we run the container we also need to know which port to expose. The port is listed in the `env` file:

```
cat env
```

The output you will get is similar to:
```
...
```
docker run -p 3306:3306 002969e1d810
```

This starts the container and you can now use it for testing the MySQL module.

To run metricbeat with the module we need to build the binary and enable the module. The assumption is now that you are back in the `beats` path:
...
This will enable the module and start Metricbeat. It's now sending data to your local Elasticsearch instance. If you need to modify the mysql config, adjust `modules.d/mysql.yml` and restart Metricbeat. To update the Elasticsearch output, adjust `metricbeat.yml` file.
