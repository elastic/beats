...
If set to true, Filebeat starts reading new files at the end
instead of the beginning. If it is used in combination with log rotation, 
it might be that the first log entries of a new file are skipped.
By default is set to false.


NOTE: This setting is meant to be used only on the first Filebeat run on a set of log files if you want to avoid indexing old log lines. After the first run, we recommend disabling this option, otherwise you risk losing lines on file rotation.
