...
<1> When multiple hosts are defined, the default behavior in Filebeat is to
    ...
    {libbeat}/configuration.html#loadbalance[loadbalance] configuration option.
<2> Note that if the `tls` section is missing, then TLS is disabled. TLS is
    ...
    specific configuration options, see
    {libbeat}/configuration.html#configuration-output-tls[TLS Options].
...
For more information about these options, see {libbeat}/configuration.html[Beats configuration options].
...
these options, see {libbeat}/command-line-options.html[Beats Command Line Options].
