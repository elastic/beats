...
If all three of `TENANT_ID`, `CLIENT_ID`, and `CLIENT_SECRET` are set, Auditbeat uses an explicit client secret credential for the ARM lookup. Otherwise, it uses the Azure SDK for Go `DefaultAzureCredential`.
