...
If set to `true`, Auditbeat will automatically reload TLS certificates, keys, and CA certificates from disk whenever they change, without requiring a restart. The default value is `true`.
...
How often Auditbeat checks whether the certificate, key, or CA files have changed on disk. The default value is `5s`.
