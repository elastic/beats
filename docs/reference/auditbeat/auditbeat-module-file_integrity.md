...
* **Windows**: Multiple backends are supported: `auto`, `fsnotify`, `etw`.
  * `auto`: {applies_to}`stack: ga 9.4.0, preview 9.2.0` Automatically tries the `etw` backend first, falling back to `fsnotify` if ETW is not available or supported.
...
::   Select the backend that will be used to source events. The available backends vary by operating system:
    ...
    * **Windows:** `auto`, `fsnotify`, `etw`.
      ...
      * {applies_to}`stack: ga 9.2.0` `fsnotify`
    * **macOS:** Only `auto` and `fsnotify` are supported.
      The default varies by version:
      * {applies_to}`stack: ga 9.4.0` `auto`
      * {applies_to}`stack: ga 9.2.0` `fsnotify`

    {applies_to}`stack: ga 9.4.0` When `auto` is used (or when left unspecified), the module automatically selects the best available backend for the platform. See the "How it works" section for details on backend selection order.
