...
mapped_pages:
  - https://www.elastic.co/guide/en/beats/auditbeat/current/processor-translate-ldap-attribute.html
applies_to:
  stack: ga
---
...
| `ad_guid_translation` | no | null | Controls Active Directory GUID binary conversion. When `null` (default), GUID conversion is enabled only when `ldap_search_attribute` is `objectGUID`. Set to `true` to always convert, or `false` to never convert. This is specific to Microsoft Active Directory; do not use for other LDAP implementations. |
