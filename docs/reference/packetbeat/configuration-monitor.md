...
Refer to [Use internal collection to send monitoring data](/reference/packetbeat/monitoring-internal-collection.md) for usage examples.
...
The time interval (in seconds) when state information is sent to the {{es}} cluster. A new snapshot of Packetbeat state is generated and scheduled for publishing each period. The default value is 60 * time.Second.
