...
A list of query parameters that Packetbeat will automatically censor in the transactions that it saves. The values associated with these parameters are replaced by `'xxxxx'`. Parameter names are matched case-insensitively. By default, no changes are made to the HTTP messages.
