...
When `loadbalance: false` is set, Packetbeat sends data to a single host at a time. Packetbeat connects to the first configured host and sends all data to that host until the connection fails, then fails over to the next configured host in order. Use the `worker` or `workers` setting to specify the number of concurrent connections to the active host. Data will still be sent as long as Packetbeat can connect to at least one of its configured hosts.
