...
- type: filestream
  ...
  # Emit one event per file update with full file snapshot in `message`.
  # This feature is beta.
  #full_content.enabled: false
  #
  # Constraints when full_content.enabled is true:
  # - `compression` must be disabled.
  # - `parsers` are not supported.
  #
  # If include_lines/exclude_lines filtering or reader truncation modifies the
  # original file contents, Filebeat sets `log.flags: ["truncated"]`.
