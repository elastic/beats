...
When the `enrich_with: ["sign_in_activity"]` option is set, an additional call is made on each full synchronization and on incremental updates that include at least one user identity change to:
