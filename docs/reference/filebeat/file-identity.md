...
| fingerprint (default in Filebeat >= 9.0) | Log files with unique content.<br><br>{applies_to}`stack: ga 9.5+` Log files of any size (enhanced fingerprint).<br><br>{applies_to}`stack: ga 8.10-9.4` Log files larger than the fingerprint size (1 KB by default). | The most stable. Support for any OS, any file system, network shares, containers and VMs.<br><br>{applies_to}`stack: ga 9.5+` Ingests files smaller than the fingerprint size without delay. | Slightly higher CPU / memory usage.<br><br>{applies_to}`stack: ga 8.10-9.4` Doesn't ingest files until they reach the fingerprint size (1 KB by default). |
...
```yaml
prospector.scanner.fingerprint:
  enabled: true # required: the scanner computes no fingerprints by default
  offset: 32    # e.g. the first 32 bytes are always identical in every file, we skip them
  length: 1024  # strong guarantee of uniqueness
file_identity.fingerprint: ~
```
...
{applies_to}`stack: ga 8.10-9.4` If a new log file appears, the filestream input doesn’t register it until it grows to 1056 bytes (`offset`+`length`). Files that don't grow to that size are not ingested.
