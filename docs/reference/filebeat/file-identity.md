...
File identity is how [filestream input](/reference/filebeat/filebeat-input-filestream.md) differentiates one file from another in order to keep track of ingestion progress on each file.
...
2. If you ingest from `*.old` - massive data duplication.

Why would this happen? File names are not stable (non-changing) identifiers. This means we cannot use names to track the progress of ingestion on each file also known as offset. The offset on `important.log` was somewhere in the middle, let's say `offset: 145400` but since it got renamed to `important.log.old`, we detect this file as new and we start ingesting from the beginning - `offset: 0`. Therefore we re-ingest the same data again.

Various file identities were introduced in Filebeat to remedy this problem using some additional file metadata to accurately track offsets in each file despite file operations applied to them.

### Why have multiple options?

File identity implementations have advantages over each other and they are optimized for a particular use-case or environment. The goal of this document is to provide a comprehensive overview that would help you to choose what fits your use-case the best.
...
If you're completely certain that your environment does not have log rotation, does not ever rename or move files, then you might consider using paths as unique stable identifiers for each file. This is the simplest but the riskiest option and it's not recommended to use.
...
You can see that the file system cached the inode value from the first file, which we deleted, and reused it for the second copy with the same filename: 1715023, 1715026, and 1715023.
...
As mentioned before, it's common for containerized environments and network shares. If you're planning to run Filebeat in such an environment, avoid using this file identity.
...
For some use-cases it might be a requirement to receive data as soon as possible without delays. For some use-cases files would not grow to such sizes at all and would never be ingested. It's important to consider these limitations accordingly.
