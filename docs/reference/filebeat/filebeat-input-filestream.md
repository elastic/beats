...
* As a technical preview feature, it can read GZIP files.
...
In general a GZIP file is treated as a plain file, with the same guarantees filestream
offers. This includes offset tracking and resuming reading from partially read files.

Filestream decompresses GZIP files on the fly, decompressing data as needed. It
...
The fingerprinting is done on the decompressed data, and log rotation works out of the box.

::::{important}
Do not configure the [`copytruncate` strategy](#_rotation_external_strategy_copytruncate) for log rotation
when ingesting GZIP files. Using `copytruncate` might lead to data loss. The
default log rotation and file identity fingerprinting are sufficient for GZIP
files.
::::
