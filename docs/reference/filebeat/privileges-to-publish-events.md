...
When using modules, Filebeat checks for ingest pipelines and loads them when it connects to {{es}}. To skip this runtime pipeline check after the required pipelines have been loaded, or when pipelines are managed separately, set:
...
1. Create a **writer role**, called something like `filebeat_writer`, that has the following privileges:
    ...
    | Cluster | `read_pipeline` | Check for ingest pipelines used by modules. Needed when using modules unless `setup.pipelines.enabled` is `false`. |
