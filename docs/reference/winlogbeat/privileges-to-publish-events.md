...
Winlogbeat checks for ingest pipelines and loads them when it connects to {{es}}. {applies_to}`stack: ga 9.6` To skip this runtime pipeline check after the required pipelines have been loaded, or when pipelines are managed separately, set:
