...
| `ldap_ssl`* | no |  | LDAP TLS/SSL connection settings. See [SSL](/reference/winlogbeat/configuration-ssl.md). |
...
1. DNS SRV lookups for `_ldaps._tcp` (preferred) and `_ldap._tcp`. The processor queries multiple patterns:
  - `_ldap._tcp.dc._msdcs.<domain>` (Active Directory domain controllers)
  - `_ldap._tcp.<domain>` (standard domain lookup)
  - If no domain is available, bare queries like `_ldap._tcp` which rely on the system's DNS search suffix configuration.
  ...
  Results are ordered by SRV priority and weighted randomly (RFC 2782) to avoid overloading a single host.
