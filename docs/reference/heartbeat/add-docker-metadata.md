...
{applies_to}`stack: ga 9.4.2+` If Docker is unavailable at startup, the processor retries the connection every `wait_for_metadata_retry_period` (default 10s). Set `wait_for_metadata` to `true` to block startup until Docker metadata is available and `wait_for_metadata_timeout` to limit the total time spent retrying.
...
`wait_for_metadata` {applies_to}`stack: ga 9.4.2+`
...
`wait_for_metadata_timeout` {applies_to}`stack: ga 9.4.2+`
...
`wait_for_metadata_retry_period` {applies_to}`stack: ga 9.4.2+`
