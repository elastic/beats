...
Sets the hostname the Beat reports in `host.name` and `observer.hostname`. Use this when the Beat cannot determine its own hostname, for example in Kubernetes environments where `os.Hostname()` returns a pod name instead of the node name.
