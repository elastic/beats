...
{applies_to}`stack: ga 9.1` By default, Windows log files are stored under `C:\Program Files\Heartbeat-Data\logs`.

For versions earlier than 9.1.0, logs are stored by default under `C:\ProgramData\heartbeat\Logs`.
