...
`billing_usage_lookback` {applies_to}`stack: ga 9.4+`
:   (*duration*, positive multiple of `24h`, default: `24h`) How far back to query usage data on each
    fetch. Azure Cost Management data is documented to be updated for up to 72 hours after a billing period closes, so increasing
    this to `72h` ensures that late cost corrections from Azure are re-queried and ingested.

`billing_forecast_window` {applies_to}`stack: ga 9.4+`
:   (*duration*, positive multiple of `24h`, default: `720h`) The length of the forecast period,
    counted forward from the forecast start date (reference time minus 2 days). Defaults to 30 days (`720h`).
