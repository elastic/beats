...
* **metadata_cache**: (`true`/`false` default `false`) Enable caching of metadata. If set to true, metadata will be cached to improve performance. {applies_to}`product: ga 9.1.0`
