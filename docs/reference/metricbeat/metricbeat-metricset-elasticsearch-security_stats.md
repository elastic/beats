...
The `/_security/stats` endpoint is only served when the Elasticsearch security feature is enabled (`xpack.security.enabled: true`). The metricset checks `GET /_xpack` on each scrape; when security is disabled it emits a throttled debug log and no events.
