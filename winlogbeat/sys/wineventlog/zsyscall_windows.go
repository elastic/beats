...
func _EvtQuery(session EvtHandle, chanelOrFilePath *uint16, query *uint16, flags EvtQueryFlag) (handle EvtHandle, err error) {
	r0, _, e1 := syscall.Syscall6(procEvtQuery.Addr(), 4, uintptr(session), uintptr(unsafe.Pointer(chanelOrFilePath)), uintptr(unsafe.Pointer(query)), uintptr(flags), 0, 0)
	handle = EvtHandle(r0)
	if handle == 0 {
		if e1 != 0 {
			err = errnoErr(e1)
...
func _EvtSeek(resultSet EvtHandle, position uint64, bookmark EvtHandle, timeout uint32, flags EvtSeekFlag) (err error) {
