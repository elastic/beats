...
func (em *EventMetadata) initEventMessage(itr *EventMetadataIterator, publisher *PublisherMetadata) error {
	...
	if err != nil {
		return fmt.Errorf("failed to get message string using message "+
			"ID %v for for event ID %v"+": %w", messageID, em.EventID, err)
...
func (em *EventMetadata) setMessage(msg string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to parse message template for "+
			"event ID %v (template='%v')"+": %w", em.EventID, msg, err)
