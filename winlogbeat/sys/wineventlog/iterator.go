	...
	for batchSize > 0 {
		...
		case windows.RPC_S_INVALID_BOUND:
			...
			if itr.subscriptionFactory != nil {
				...
				if err != nil {
					itr.lastErr = fmt.Errorf("failed in EvtNext while trying to "+
						"recover from RPC_S_INVALID_BOUND error"+": %w", err)
				...
				itr.lastErr = fmt.Errorf("failed in EvtNext (try "+
					"reducing the batch size or providing a subscription "+
					"factory for automatic recovery)"+": %w", err)
