...
func (l *winEventLog) Read() ([]Record, error) {
	...
	for _, h := range handles {
		...
		if r.Message == "" {
			...
			if err != nil {
				logp.Warn("%s error salvaging message (event id=%d qualifier=%d created at %s will be included without a message): %v",
					l.logPrefix, r.EventIdentifier.ID, r.EventIdentifier.Qualifiers, r.TimeCreated.SystemTime, err)
			}
		}
