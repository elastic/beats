...
that is not an Administrator, then follow the below steps. The local user account 
...
. Under `Log On` tab, select `This account:` and browse for your local account user 
that you want to run {beatname_uc} service.
