...
The maximum size of the message displayed in the request or response fields,
in bytes. If the message is bigger than the size, it will be truncated. Use this
option to avoid publishing huge messages. It is useless if one of
send_request or send_response options is not activated. Default is 1000 bytes.

===== parse_headers

Parse the additional arguments specified in the headers field of a message.
Those arguments are key-value pairs provided by users of the protocol,
and can specify informations like the content type of the message or the
message priority. Default is true.

===== parse_arguments

Parse the additional arguments specified in AMQP methods. Those arguments
are key-value pairs specified by the user, and can be of any length. Default
is true.

===== hide_connection_information

If set to false, the connection layer methods of the protocol will also be
displayed, like the opening and closing of connections and channels by clients,
or the quality of service negociation. Default is true.
