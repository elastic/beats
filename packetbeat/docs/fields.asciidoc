...
[float]
=== `tls.client_hello.timestamp`

type: long

The current time and date in standard UNIX 32-bit format according to the client's internal clock.
...
[float]
=== `tls.server_hello.timestamp`

type: long

The current time and date in standard UNIX 32-bit format according to the server's internal clock.
...
The certificate's serial number, in hexadecimal.
