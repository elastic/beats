...
Information about the SMTP transaction.


*`smtp.type`*::
+
--
The SMTP transaction type (`PROMPT`, `COMMAND` or `MAIL`).
...
SMTP request (only `COMMAND` transactions).
...
SMTP response (only `COMMAND` and `PROMPT` transactions)
...
+
--
The SMTP status phrases (one per each line of the response)

type: object

--

*`smtp.envelope_sender`*::
+
--
