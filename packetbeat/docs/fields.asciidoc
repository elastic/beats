...
Generated by the client, allows the client to correctly match requests and responses.
...
The gateway IP address used by the client to contact the server.
...
The client's hardware (layer two) address.
...
Optional, for DHCPOFFER or DHCPACK messages, the name of the server sending the message.
...
The general type of DHCP message.
...
The specific type of DHCP message being sent (DHCPOFFER, DCHPREQUEST, etc)
