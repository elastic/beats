...
OrientDB-specific event fields. These fields mirror closely the fields for the OrientDB wire protocol. The higher level fields (for example, `query` and `resource`) apply to OrientDB events as well.
...
+
--
If the OrientDB command has resulted in an error, this field contains the error message returned by the OrientDB server.
