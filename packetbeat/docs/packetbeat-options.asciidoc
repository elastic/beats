...
[[packetbeat-smtp-options]]
=== Capture SMTP traffic

++++
<titleabbrev>SMTP</titleabbrev>
++++

The `smtp` section of the +{beatname_lc}.yml+ config file specifies configuration options for the SMTP
protocol. Here is a sample configuration:
...
If set to true, Packetbeat parses the SMTP DATA payload headers and includes them
in published events.  Those headers are key-value pairs that specify information
such as the subject of the message or the message recipient. The default is false.

===== `send_data_body`

If set to true, Packetbeat includes the raw SMTP DATA payload body (contents of the message)
in published events. The default is false
