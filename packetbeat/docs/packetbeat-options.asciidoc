...
for limiting the amount of data Packetbeat indexes in its certificate fields.

Also see <<common-protocol-options>>.

===== `send_certificates`

If the certificates presented by the client and server must be included in the
event. The server's certificate is indexed in the `server_certificate` field and
its certification chain in the `server_certificate_chain`. For the client,
`client_certificate` and `client_certificate_chain` are used.

The default is `true`.

===== `include_raw_certificates`

In addition to a fields representation of each certificate, include also the
raw certificate encoded in PEM format, as a `raw` field under each certificate.

Default is `false`. This setting requires that the `send_certificates` option
is set to `true`, otherwise its ignored.
