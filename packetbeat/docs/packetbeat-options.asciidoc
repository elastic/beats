...
[[configuration-packetbeat-kafka]]
==== Kafka Configuration Options

The `kafka` section of the +{beatname_lc}.yml+ config file specifies configuration options for handling the Kafka protocol. The Kafka protocol analyzer supports processing all public Kafka Network API from Kafka 0.8 up to 0.10 (protocol versions 0, 1, and 2). Here is a sample configuration section for Kafka:

------------------------------------------------------------------------------
packetbeat.protocols.kafka:
  ports: [9092]
  send_details: true
------------------------------------------------------------------------------
...
List of transaction types to ignore. Messages for transaction types configured will not be parsed by the protocol analyzer.

Valid Transaction types are: `Produce`, `Fetch`, `Offset`, `Metadata`, `Internal4`, `Internal5`, `Internal6`, `Internal7`, `OffsetCommit`, `OffsetFetch`, `GroupCoordinator`, `JoinGroup`, `Heartbeat`, `LeaveGroup`, `SyncGroup`, `DescribeGroups`, and `ListGroups`.

By default all transactions will be reported.

===== send_details

If this option is enabled, the kafka messages will be fully parsed, such that the events being published will contain request and response parameters. Due to the nature of the Kafka protocol batching up multiple queries into one Transaction, sending details can produce multiple events per Transaction. Cohesive events will share the same `transaction_id` generated by packetbeat.

If this option is disabled, only the transaction type will be reported.
