...
// the Module will be published to one of new publisher.Clients generated from the
// pubClientFactory.
func NewRunner(clients map[string]beat.Client, mod *Wrapper) Runner {
	return &runner{
		logger:  logp.NewLogger("runner"),
...
// NewRunnerForStaticModule returns a Runner facade. The events generated by
// the Module will be published to a new publisher.Client generated from the
// pubClientFactory.
func NewRunnerForStaticModule(client beat.Client, mod *Wrapper) Runner {
	return &runner{
		logger:  logp.NewLogger("runner"),
		done:    make(chan struct{}),
		mod:     mod,
		clients: map[string]beat.Client{staticModuleSink: client},
	}
}
...
func (mr *runner) Start() {
	mr.startOnce.Do(func() {
		...
		go func() {
			defer mr.wg.Done()
			PublishChannels(mr.clients, output)
		}()
	})
}
...
func (mr *runner) Stop() {
	mr.stopOnce.Do(func() {
		...
		for metricSetName, client := range mr.clients {
			err := client.Close()
			if err != nil {
				mr.logger.Errorf("Error occurred while stopping runner (name: %s): %v", metricSetName, err)
			}
		}
		mr.wg.Wait()
