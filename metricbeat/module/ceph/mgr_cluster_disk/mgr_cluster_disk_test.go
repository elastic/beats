...
func TestFetchEventContents(t *testing.T) {
	...
	response, err := ioutil.ReadFile(absPath + "/mgr_df_sample_response.json")
	assert.NoError(t, err)
	...
	assert.EqualValues(t, uint64(3801088), used["bytes"])

	total := event["total"].(common.MapStr)
	assert.EqualValues(t, uint64(10737418240), total["bytes"])

	available := event["available"].(common.MapStr)
	assert.EqualValues(t, uint64(9659875328), available["bytes"])
}
