...
func TestFetchEventContents(t *testing.T) {
	...
	response, err := ioutil.ReadFile(absPath + "/mgr_osd_perf_sample_response.json")
	assert.NoError(t, err)
	...
	assert.EqualValues(t, 2, stats["apply_latency_ms"])
	assert.EqualValues(t, 4, stats["apply_latency_ns"])
	assert.EqualValues(t, 1, stats["commit_latency_ms"])
	assert.EqualValues(t, 3, stats["commit_latency_ns"])
}
