...
it creates separate events for each element in the array. If each *element* in the array is not a valid `json object` by itself (like a string), the event created for this will have the *array* assigned to the key `_data` like this: `{ "_data": ["element1", "element2" ..]" }`.
