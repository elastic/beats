...
func TestRedis(t *testing.T) {
	mtest.Runner.Run(t, compose.Suite{
		"FetchRedisInfo": testFetchRedisInfo,
		"FetchKeys":      testFetchKeys,
		"FetchKeyInfo":   testFetchKeyInfo,
	})
}

func testFetchRedisInfo(t *testing.T, r compose.R) {
	conn, err := rd.Dial("tcp", r.Host())
	if err != nil {
		t.Fatal("connect", err)
	}
	defer conn.Close()
...
func testFetchKeys(t *testing.T, r compose.R) {
	conn, err := rd.Dial("tcp", r.Host())
	if err != nil {
		t.Fatal("connect", err)
	}
...
func testFetchKeyInfo(t *testing.T, r compose.R) {
	conn, err := rd.Dial("tcp", r.Host())
	if err != nil {
		t.Fatal("connect", err)
	}
