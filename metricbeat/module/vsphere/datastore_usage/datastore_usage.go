...
func (m *MetricSet) Fetch() ([]common.MapStr, error) {
	...
	for _, dc := range dcs {
		...
		for _, ds := range dst {
			...
			usedSpacePercent := 100 * (float64(ds.Summary.Capacity) - float64(ds.Summary.FreeSpace)) / float64(ds.Summary.Capacity)
			usedSpaceBytes := float64(ds.Summary.Capacity) - float64(ds.Summary.FreeSpace)
			...
			event := common.MapStr{
				...
				"freespace": common.MapStr{
					"bytes": ds.Summary.FreeSpace,
				},
				"usedspace": common.MapStr{
