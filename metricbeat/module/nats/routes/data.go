...
// eventsMapping maps the top-level routes metrics AND also per-route metrics
func eventsMapping(r mb.ReporterV2, content []byte) error {
	...
	for _, con := range connections.Routes {
		...
		con["num_routes"] = connections.RoutesNum
	...
	if len(connections.Routes) == 0 {
		var evt mb.Event
		con := make(map[string]interface{})
		con["server_id"] = connections.ServerID
		con["now"] = connections.Now
		con["num_routes"] = connections.RoutesNum
		evt, err = eventMapping(con, emptyRoutesSchema)
		if err != nil {
			r.Error(errors.Wrap(err, "error mapping connection event"))
			return nil
		}
