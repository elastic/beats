// Package pluszone reads server zone status from Nginx, ngx_http_status_module is required.
package pluszone
...
const (
	...
	// defaultPath is the default path to the ngx_http_status_module server zone endpoint on Nginx.
	defaultPath = "/status/server_zones"
)
...
func init() {
	if err := mb.Registry.AddMetricSet("nginx", "pluszone", New); err != nil {
		panic(err)
	}
}
...
type MetricSet struct {
	...
	url    string       // Nginx pluszone endpoint URL.
...
// Fetch makes an HTTP request to fetch status metrics from the pluszone endpoint.
