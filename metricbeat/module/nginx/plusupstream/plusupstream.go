// Package plusupstream reads server upstream status from Nginx, ngx_http_status_module is required.
...
const (
	...
	// defaultPath is the default path to the ngx_http_status_module server upstream endpoint on Nginx.
	defaultPath = "/status/upstreams"
)
...
func init() {
	if err := mb.Registry.AddMetricSet("nginx", "plusupstream", New); err != nil {
		panic(err)
	}
}
...
type MetricSet struct {
	...
	url    string       // Nginx plusupstream endpoint URL.
...
func New(base mb.BaseMetricSet) (mb.MetricSet, error) {
	...
	debugf("nginx-plusupstream URL=%s", u)
	return &MetricSet{
		BaseMetricSet: base,
		url:           u.String(),
...
// Fetch makes an HTTP request to fetch status metrics from the plusupstream endpoint.
