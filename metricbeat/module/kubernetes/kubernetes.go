...
func (m *module) GetKubeletStats(http *helper.HTTP) ([]byte, error) {
	...
	errorUnathorisedMsg := fmt.Sprintf("HTTP error %d", httpnet.StatusUnauthorized)
	if statsCache.lastFetchErr != nil && strings.Contains(statsCache.lastFetchErr.Error(), errorUnathorisedMsg) {
