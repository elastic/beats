...
func New(base mb.BaseMetricSet) (mb.MetricSet, error) {
	...
	watcher, err := kubernetes.NewWatcher(client, config.SyncPeriod, "", config.Namespace, &kubernetes.Event{})
	if err != nil {
		logp.Err("kubernetes: Couldn't create watcher for %t", &kubernetes.Event{})
		return nil, err
	}
...
func (m *MetricSet) Run(reporter mb.PushReporter) {
	...
	handler := kubernetes.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(_, new kubernetes.Resource) {
			reporter.Event(generateMapStrFromEvent(new.(*kubernetes.Event)))
		},
