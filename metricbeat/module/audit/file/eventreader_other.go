...
func NewEventReader(c Config) (EventReader, error) {
	...
	return &reader{watcher, make(chan Event, 1), make(chan error, 1)}, nil
}
	...
	go func() {
		...
		for {
			...
			case event := <-r.watcher.Events:
				r.outC <- convertToFileEvent(event)
...
func convertToFileEvent(e fsnotify.Event) Event {
	event := Event{
		Timestamp:  time.Now(),
		TargetPath: e.Name,
