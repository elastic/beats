...
/*
Test the resulted metrics family when ParseMetricFamilies uses a PromParser. Accepted types:
https://github.com/prometheus/prometheus/blob/b0944590a1c9a6b35dc5a696869f75f422b107a1/pkg/textparse/promparse.go#L291-L304
Type of parser is determined based on the content used.
*/
func TestParseMetricsFamilyPromParser(t *testing.T) {
	stringp := func(x string) *string { return &x }
	float64p := func(x float64) *float64 { return &x }
	uint64p := func(x uint64) *uint64 { return &x }
	int64p := func(x int64) *int64 { return &x }

	var expected []*MetricFamily
...
jvm_buffer_count_buffers{id="mapped - 'non-volatile memory'"} 0.0`
	gaugeMetricFamily := MetricFamily{
		Name: stringp("jvm_buffer_count_buffers"),
		Help: stringp("An estimate of the number of buffers in the pool"),
		Type: "gauge",
		Unit: nil,
		Metric: []*OpenMetric{
			{
				Label: []*labels.Label{
					{
						Name:  "id",
						Value: "mapped - 'non-volatile memory'",
					},
				},
				Name: stringp("jvm_buffer_count_buffers"),
				Gauge: &Gauge{
	...
	expected = append(expected, &gaugeMetricFamily)

	counter := `
# HELP cc_seconds_total A counter
# TYPE cc_seconds_total counter
cc_seconds_total 1.0`
	counterMetricFamily := MetricFamily{
		Name: stringp("cc_seconds_total"),
		Help: stringp("A counter"),
		Type: "counter",
		Unit: nil,
		Metric: []*OpenMetric{
			{
	...
	expected = append(expected, &counterMetricFamily)

	summary := `
# HELP rpc_duration_seconds A summary of the RPC duration in seconds.
# TYPE rpc_duration_seconds summary
rpc_duration_seconds{quantile="0.01"} 3102 123
rpc_duration_seconds{quantile="0.5"} 4773 123
rpc_duration_seconds{quantile="0.99"} 76656 123
rpc_duration_seconds_sum 1.7560473e+07 123
rpc_duration_seconds_count 2693 123`
	summaryMetricFamily := MetricFamily{
		Name: stringp("rpc_duration_seconds"),
		Help: stringp("A summary of the RPC duration in seconds."),
		Type: "summary",
		Unit: nil,
		Metric: []*OpenMetric{
			{
				...
				Name:  stringp("rpc_duration_seconds"),
				Summary: &Summary{
					SampleCount: uint64p(2693),
					SampleSum:   float64p(1.7560473e+07),
					Quantile: []*Quantile{
						{
							Quantile: float64p(0.01),
							Value:    float64p(3102),
						},
						{
						...
						},
						{
							...
							Value:    float64p(76656),
						},
					},
				},
				TimestampMs: int64p(123),
			},
		},
	}
	expected = append(expected, &summaryMetricFamily)
...
http_server_requests_seconds_created{exception="None",uri="/actuator/prometheus"} 0.046745444`
	histogramMetricFamily := MetricFamily{
	...
	expected = append(expected, &histogramMetricFamily)

	undefined := `
# HELP redis_connected_clients Redis connected clients
# TYPE redis_connected_clients undefined
redis_connected_clients{instance="rough-snowflake-web"} 10.0`

	allMetrics := strings.Join([]string{gauge, counter, summary, histogram, undefined}, "\n")
	b := []byte(allMetrics)
...
/*
Test the resulted metrics family when ParseMetricFamilies uses an OpenMetricsParser: accepted types
https://github.com/prometheus/prometheus/blob/b0944590a1c9a6b35dc5a696869f75f422b107a1/pkg/textparse/openmetricsparse.go#L261-L280
Type of parser is determined based on the content used.
*/
func TestParseMetricsFamilyOpenMetricsParser(t *testing.T) {
	stringp := func(x string) *string { return &x }
	float64p := func(x float64) *float64 { return &x }
	uint64p := func(x uint64) *uint64 { return &x }
	int64p := func(x int64) *int64 { return &x }
	...
	gauge := `# HELP jvm_buffer_count_buffers An estimate of the number of buffers in the pool
# TYPE jvm_buffer_count_buffers gauge
jvm_buffer_count_buffers{id="mapped - 'non-volatile memory'"} 0.0`
	gaugeMetricFamily := MetricFamily{
		Name: stringp("jvm_buffer_count_buffers"),
	...
	counter := `# HELP cc_seconds A counter
# TYPE cc_seconds counter
# UNIT cc_seconds seconds
cc_seconds_total 1.0
cc_seconds_created 123.456`
	counterMetricFamily := MetricFamily{
		Name: stringp("cc_seconds"),
		Help: stringp("A counter"),
		Type: "counter",
		Unit: stringp("seconds"),
		Metric: []*OpenMetric{
			{
				Label: []*labels.Label{},
				Name:  stringp("cc_seconds_total"),
			...
			{
				Label: []*labels.Label{},
				Name:  stringp("cc_seconds_created"),
				Counter: &Counter{
					Value: float64p(123.456),
				},
			},
		},
	}
		...
		Metric: []*OpenMetric{
			{
				...
				TimestampMs: int64p(2000),
			},
		},
	}
	expected = append(expected, &infoMetricFamily)

	stateset :=
		`# TYPE some_name stateset
some_name{a="bar"} 2`
	statesetMetricFamily := MetricFamily{
		Name: stringp("some_name"),
		Help: nil,
		Type: "stateset",
		Unit: nil,
		Metric: []*OpenMetric{
	...
	expected = append(expected, &statesetMetricFamily)
...
rpc_duration_seconds_created 2693 1`
	summaryMetricFamily := MetricFamily{
		Name: stringp("rpc_duration_seconds"),
		Help: stringp("A summary of the RPC duration in seconds."),
		Type: "summary",
...
http_server_requests_seconds_created{exception="None",uri="/actuator/prometheus"} 0.046745444`
	...
	gaugeHistogram := `# TYPE ggh gaugehistogram
ggh_bucket{le=".9"} 2
ggh_gcount 2
ggh_gsum 1`
	gaugeHistogramMetricFamily := MetricFamily{
		Name: stringp("ggh"),
		Help: nil,
		Type: "gaugehistogram",
		Unit: nil,
	...
	expected = append(expected, &gaugeHistogramMetricFamily)

	unknown := `# HELP redis_connected_clients Redis connected clients
# TYPE redis_connected_clients unknown
redis_connected_clients{instance="rough-snowflake-web"} 10.0`
	unknownMetricFamily := MetricFamily{
	...
	expected = append(expected, &unknownMetricFamily)

	undefined := `# HELP something Undefined metric.
# TYPE something undefined
something{instance="rough-snowflake-web"} 10.0
# EOF`

	allMetrics := strings.Join([]string{gauge, counter, info, stateset, summary, histogram, gaugeHistogram, unknown, undefined}, "\n")
	b := []byte(allMetrics)
	result, err := ParseMetricFamilies(b, OpenMetricsType, time.Now())
	if err != nil {
		t.Fatalf("ParseMetricFamilies for content type %s returned an error.", ContentTypeTextFormat)
	}
	assert.ElementsMatch(t, expected, result)
