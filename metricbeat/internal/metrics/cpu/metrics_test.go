...
// TestCpuinfoScanner tests the parsing of `/proc/cpuinfo` for different
...
func TestCpuinfoScanner(t *testing.T) {
	testCases := []string{
		"cpuinfo",
		"cpuinfo-quad-socket",
		"armv7", // Source: https://github.com/util-linux/util-linux/blob/master/tests/ts/lscpu/dumps/armv7.tar.gz
	}
	for _, tc := range testCases {
		t.Run(tc, func(t *testing.T) {
			...
			cpuInfo, err := cpuinfoScanner(scanner)
			if err != nil {
				t.Fatalf("cpuinfoScanner error: %s", err)
			}
			...
			if generateGoldenFile, _ := strconv.ParseBool(os.Getenv("GENERATE")); generateGoldenFile {
				...
				cpuinfoScannerGoldenFile(t, cpuInfo, tc)
				return
			}
...
func cpuinfoScannerGoldenFile(t *testing.T, data []CPUInfo, name string) {
