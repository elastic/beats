...
stringified JSON of the input configuration. `raw` overrides every other hint and can be used to create either a single configuration or
...
parent container on port `9090` at a 1 minute interval. If you need more than one entry for
values, use comma separated strings. That is:

["source","yaml",subs="attributes"]
-------------------------------------------------------------------------------------
co.elastic.metrics/metricsets: 'metricset_1,metricset_2'
co.elastic.metrics/hosts: 'host_1,host_2'
-------------------------------------------------------------------------------------
