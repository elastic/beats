...
The Kafka Metricbeat module has two types of metrics. Those who are related with the cluster and can be present across all metricset which are usually called module level metrics. And those metrics that are only present using a specific metricset for example, the `broker` metricset.
...
Kafka module level metrics. These metrics aren't related with a particular metricset and can be present in any of them
...
Module level Broker Consumer Group Information that have been read from (Broker handling the consumer group).
...
Metrics about the broker that the partition belongs to



*`kafka.partition.broker.broker.id`*::
+
--
...
*`kafka.partition.broker.broker.address`*::
+
--
...
Metrics about the topics in the exposed partition
...
Overall metrics of the partitions
