...
+
--
Network received rate bytes per second, calculated with dividing network.in.bytes by 300 or by 60(detailed monitoring).
...
+
--
Network sent out rate bytes per second, calculated with dividing network.out.bytes by 300 or by 60(detailed monitoring).
...
+
--
Instance store volumes read rate bytes per second, calculated with dividing diskio.read.bytes by 300 or by 60(detailed monitoring).
...
+
--
Instance store volumes write rate bytes per second, calculated with dividing diskio.write.bytes by 300 or by 60(detailed monitoring).
...
+
--
Read operations count per second, calculated with dividing diskio.read.ops by 300 or by 60(detailed monitoring).
...
+
--
Write operations count per second, calculated with dividing diskio.write.ops by 300 or by 60(detailed monitoring).
