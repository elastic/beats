...
*`logstash.pipeline.ephemeral_id`*::
+
--
type: keyword

Pipeline Ephemeral ID
...
Number of processor (filter+output) workers


--

*`logstash.pipeline.batch_size`*::
+
--
type: integer

Max. number of events consumed by each processor worker
