...
+
--
The total virtual memory the process has. On Windows os's, this represents the Commit Charge (the total amount of memory that the memory manager has committed for a running process) value in bytes for this process.
...
+
--
The Resident Set Size. The amount of memory the process occupied in main memory (RAM). On Windows os's this represents the current working set size, in bytes.
