...
`resource` contains the metrics that reports what services are running on a specific AWS account.
...
+
--
The resource name in AWS. The content of this part of the ARN varies by service. It often includes an indicator of the type of resource followed by a slash (/) or a colon (:), followed by the resource name itself.
