...
--
<<<<<<< HEAD
The cloud account or organization id used to identify different entities in a multi-tenant environment. - name: cloudwatch
  type: group
  description: >
    `cloudwatch` contains the metrics that were scraped from AWS CloudWatch which contains monitoring metrics sent by different namespaces.
  release: beta
...
=======
SQS queue name
>>>>>>> remove duplicate cloud.account.id field
