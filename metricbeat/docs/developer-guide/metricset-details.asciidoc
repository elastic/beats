...
If a request must be ended or has an error, make sure that you return a good error
message. This error message is also sent to Elasticsearch, making it possible to
not only fetch metrics from the service, but also report potential problems or errors
with the metricset.
...
//REVIEWERS: Not sure about the above edit, but the original sentence was too vague.
...
`makeunit-tests` or for integration tests `make integration-tests`. Be aware that
