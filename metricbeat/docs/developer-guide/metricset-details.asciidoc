...
written in Python based on our small Python test framework. We use `virtualenv` to deal with Python dependences,
it’s simply to use command `make python-env` and then `. build/python-env/bin/activate`.
...
Sometimes you may just want to run specify single integration test, such as `apache` module, firstly, start the docker service by run
`docker-compose run -p port:port apache` (if your module doesn't need a docker service like `golang` module, you can just ignore this)
and then run `cd tests/system`, as the integration test is under this folder, and then run `INTEGRATION_TESTS=true nosetests test_apache.py`,
remember to replace `test_apache.py` to your own test file.
