...
In this guide, you learn how to create your own metricset. If you want to create
...
[float]
=== Creating Metricset Files
Creating a new metricset is simple. Run the following command inside your beat directory:
...
The metricset that you created is already a functioning metricset and can be compiled. To compile your new metricset, run the following:
...
The first command, `make collect`, updates all generated files with the most recent metricset. The second command,
...
[float]
== Metricset Files
You'll find the metricset, along with its generated files, under `module/{module}/{metricset}`. This directory
...
Let's look at each file in more detail next.
...
If an error happens, the error must be returned and then sent instead
...
The following example shows a counter that is incremented by one for each `Fetch` call:

//REVIEWERS: Wondering if there is something missing here. The previous sentence seems like a non sequitur.
