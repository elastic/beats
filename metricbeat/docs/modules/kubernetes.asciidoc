...
Some of the previous components are running in each of the Kubernetes nodes (like `kubelet` or `proxy`) while others provide a single cluster-wide endpoint. This is important to determine the optimal configuration and run strategy for the metricsets.
...
This section highlights some groups of metricsets with similar endpoint access needs. For metricsets not mentioned in this section please access individual metricsets documents for more details (***include LINK here to the end of the page***)

==== node / system / pod / container / module

The default metricsets are `container`, `node`, `pod`, `system` and `volume`, and require access to the Kubelet endpoint in each of the nodes.

Depending on the version and configuration of Kubernetes nodes, `kubelet` might provide a read only http port (typically 10255), which is used in some configuration examples. But in general, and lately, this endpoint requires SSL / https access (to port 10250 by default) and authentication via token.
...
Kube-state-metrics is not installed by default in Kubernetes, so for this metricsets to work it should be installed (*** add link ***).
