...
The default metricsets are `container`, `node`, `pod`, `system` and `volume`, which require `hosts` field pointing to the kubelet service of the node.

All metricsets with the `state_` prefix require `hosts` field pointing to kube-state-metrics
service within the cluster, while the rest should be pointing to other services, usually identified by its name. Check the
example configuration below and the individual metricsets documents for more details.
