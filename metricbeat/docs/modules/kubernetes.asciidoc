...
The default metricsets are `container`, `node`, `pod`, `system` and `volume`, which should point to the kubelet service of the node.

All metricsets with the `state_` prefix require `hosts` field pointing to kube-state-metrics
service within the cluster, while the rest should be pointed to other services, usually identified by its name. Check the
example configuration above and the individual metricsets documents for more details.

For a complete reference on how to configure and run this module in Kubernetes as part of a `DaemonSet` and a `Deployment`, there's a complete example manifest available in <<running-on-kubernetes,Running Metricbeat on Kubernetes>> document.
