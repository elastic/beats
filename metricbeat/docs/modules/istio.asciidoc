...
When using Istio `1.4-`, then the `mesh`, `mixer`, `pilot`, `galley`, `citadel` metricsets should be used. The Istio
module collects metrics from these
...
When using Istio `1.5+`, the `istiod` and `proxy` metricsets should be used. The `istiod` collects metrics directly from Istio Daemon Prometheus Exporter while `proxy` collects from the Prometheus Exporter of each of the proxy sidecars.

The default metricsets are `mesh`, `mixer`, `pilot`, `galley`, `citadel`. If using Istio `1.6+`, you should specify the metricsets `istiod` and `proxy`.
...
Example configuration for Istio `1.5+`:
[source,yaml]
----
metricbeat.modules:
...
Example configuration for Istio `1.4-`:
