...
------------------------------------------------
curl -L -O https://download.elastic.co/beats/metricbeat/metricbeat_{version}_amd64.deb
sudo dpkg -i metricbeat_{version}_amd64.deb
------------------------------------------------
...
["source","sh",subs="attributes,callouts"]
------------------------------------------------
curl -L -O https://download.elastic.co/beats/metricbeat/metricbeat-{version}-darwin.tgz
tar xzvf metricbeat-{version}-darwin.tgz
------------------------------------------------
...
Here is a sample of the `metricbeat.yml` file. Metricbeat uses modules to
collect metrics, and each module must be individually configured. The example
below shows how to configure the Apache HTTPD module.
...
////
TODO: Migrate the documentation ruflin has written to here.
////

. Define the metricbeat modules that you want enabled.
