...
Users can download and deploy this Daemonset:
["source", "sh", subs="attributes"]
------------------------------------------------
curl -L -O https://raw.githubusercontent.com/elastic/beats/{branch}/deploy/kubernetes/metricbeat-leaderelection-kubernetes.yaml
kubectl create -f metricbeat-leaderelection-kubernetes.yaml
------------------------------------------------
