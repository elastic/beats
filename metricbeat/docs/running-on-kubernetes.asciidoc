...
Note: If you are upgrading from older versions of these manifests please make sure that there are no redundant parts
as left-overs from previous versions. Deployment specification and its ConfigMaps might be the case.
...
In some cases, users might want to monitor really big Kubernetes clusters. In such cases,
...
for the Daemonset and instead deploy a singleton Metribceat instance using a Deployment with the
