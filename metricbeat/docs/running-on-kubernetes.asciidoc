...
Users can enable the respective parts of the Daemonset's ConfigMap and
set the `replicas` of the Deployment to `0` in order to configure the Daemonset
to collecting cluster-wide metrics via the leader Pod:
["source", "sh", subs="attributes"]
------------------------------------------------
curl -L -O https://raw.githubusercontent.com/elastic/beats/{branch}/deploy/kubernetes/metricbeat-kubernetes.yaml
kubectl create -f metricbeat-kubernetes.yaml
