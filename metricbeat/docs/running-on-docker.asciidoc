...
docker run \
  ...
  --net=host \ <4>
  --user root --cap-add sys_ptrace --cap-add dac_read_search \ <5>
...
files Metricbeat must be run as root in a container with `sys_ptrace` and
`dac_read_search` capabilities. These options are not needed if this metricset
is not used.
