...
func (rs *job) Generate(obj kubernetes.Resource, opts ...FieldOptions) mapstr.M {
...
func (rs *job) GenerateECS(obj kubernetes.Resource) mapstr.M {
	return rs.resource.GenerateECS(obj)
}

// GenerateK8s generates namespace metadata from a resource object
func (rs *job) GenerateK8s(obj kubernetes.Resource, opts ...FieldOptions) mapstr.M {
	_, ok := obj.(*kubernetes.Job)
	if !ok {
		return nil
	}

	meta := rs.resource.GenerateK8s("job", obj, opts...)
	return meta
}

// GenerateFromName generates pod metadata from a namespace name
func (rs *job) GenerateFromName(name string, opts ...FieldOptions) mapstr.M {
	if rs.store == nil {
		return nil
	}
	...
	if obj, ok, _ := rs.store.GetByKey(name); ok {
		...
		return rs.GenerateK8s(job, opts...)
	}
