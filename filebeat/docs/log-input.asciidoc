...
<<configuration-filebeat-options,configuration settings>> (such as `fields`,
...
If <<multiline,multiline>> is also specified, each multiline message is combined
into a single line before the lines are filtered by `exclude_lines`.
...
If <<multiline,multiline>> is also specified, each multiline message is combined
into a single line before the lines are filtered by `include_lines`.
...
The following example exports all Apache log lines except the debugging messages
(DBGs):
...
objects, like it happens for example with Docker.
...
// REVIEWERS Not sure what we are trying to say here. ^^
