...
In case filebeat is configured with kubernetes autodiscover provider (suggested configuration), kubernetes metadata
are collected by the provider internal mechanism. It uses watchers which watch for pod resources
in the cluster and collects all container IDs and relevant information(labels, annotations, namespace info, node info etc.).
For each one of the unique container IDs, a unique filestream configuration block is generated by replacing the dynamic variables `${data.kubernetes.container.id}` with each container's ID.
Then filestrean input, parses the log file of each configuration block knowing to which actual container it belongs to. 
Before publishing the log event, it enriches it with all the collected information and metadata.
              ...
              - type: filestream
                ...
                processors:
                  ...
                  - add_fields:
                      target: filebeat
                      fields:
                        template: glencore-logIngestPipelineSuffix
                paths:
                  - /var/log/containers/*-${data.kubernetes.container.id}.log
------------------------------------------------

In case filebeat is configured with filestream input, kubernetes metadata are collected using the `add_kubernetes_metadata` processor. The processor, using matchers, tries to associate a log file detected by filestream input with an actual container.
It achieves that, by matching the container ID which is part of a log file name and a container ID from the ones that run in the
kubernetes cluster. In order to track all the pods and their containers running inside the kubernetes cluster, it leverages the watchers mechanism by starting a pod, namespace and node watchers.
