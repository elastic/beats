...
Autodiscover provider a way to control whether log harvesting is by default disabled for the pods/containers when auto-discovery is in use. To enable it, just set `default.disable` to true:
...
Autodiscover provider a way to control whether log harvesting is by default disabled for the 
