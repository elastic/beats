...
Before migrating your config files, we recommend that you first read the <<configuring-howto-filebeat>>
...
The Filebeat configuration gives you more control over how each input behaves
by allowing you to configure options that were previously global in Logstash Forwarder
and set them separately for each input. See <<configuring-howto-filebeat>>.
...
For more information about these options, see <<configuring-howto-filebeat>>.
