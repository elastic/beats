...
*`fingerprint`*:: To identify files based on their first bytes of content.

This option is the most stable file identity, however it's slower than the rest of the options
and requires files to have different content in the given range of bytes.

Normally, log lines contain timestamps and other unique fields that should let to use this option
but in every use-case users should inspect their logs and determine what are appropriate values for
the `offset` and `length` parameters. Default `offset` is 0 and default `length` is `1024` or 1 KB.

WARNING: This strategy delays ingesting new files until they grow to `offset`+`length` bytes and can be
fingerprinted.

WARNING: Changing either the `length` or `offset` parameter will lead to re-ingesting all the existing files and data duplication.
