...
[float]
[id="{beatname_lc}-input-{type}-read-until-eof"]
===== `read_until_eof`
...
The shutdown signal can come from {beatname_uc} reloading its
...
This option does not change {beatname_uc}'s event delivery guarantees. The
