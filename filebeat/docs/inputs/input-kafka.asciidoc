...
[id="{beatname_lc}-input-{type}-azure"]
==== Configuring for Azure Event Hubs

The `kafka` input can be used to ingest data from Microsoft Azure Event Hubs
that have Kafka compatibility enabled, by configuring as follows:
