...
WARNING: In order to use this file identity option, one must use a filestream input with
enabled <<{beatname_lc}-input-filestream-scan-fingerprint,fingerprint option in its scanner>>. Once this file identity is enabled, changing the fingerprint configuration (offset, length, etc) will lead to a global re-ingestion of all files that match the paths configuration of the input.
