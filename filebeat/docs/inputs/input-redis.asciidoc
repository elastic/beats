...
Use the `redis` input to read entries from Redis slowlogs and acl_logs.
...
How often {beatname_uc} reads entries from Redis slowlogs and acl_logs. Specify `1s` to scan
Redis as frequently as possible without causing {beatname_uc} to scan too
frequently. Do not set this value to less than `1s`.

The default is `10s`.

IMPORTANT: Redis slowlogs and acl_logs are not permanent. To ensure that all slowlog and acl_logs entries
