...
The Wolfi docker image variant does not contain the `jouranlctl` binary.
...
* A file path
* A directory path (to collect events from all journals in a
* directory). If you specify a directory, Filebeat merges all journals
* under the directory into a single journal and reads them.

If no paths are specified, {beatname_uc} reads from the default
journal.
