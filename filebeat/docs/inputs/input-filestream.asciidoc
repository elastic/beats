...
3. The order of `parsers` is configurable. So it is possible to parse JSON lines and than
...
has to serialize the complete registry on each ACK from the outputs. Making the registry updates
much quicker.
...
7. Stale entries can be removed stale from registry, even if there is no active input.
