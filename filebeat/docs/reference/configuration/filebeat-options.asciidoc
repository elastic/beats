...
The compression feature can be used to read `.gzip` files by setting it compression to `gzip`. One prospector can only read one compression
type. This feature is mainly recommended to backfill logs in combination with the `-once` flag.

For the limitations of this feature please check https://github.com/elastic/beats/pull/3070[#3070].
