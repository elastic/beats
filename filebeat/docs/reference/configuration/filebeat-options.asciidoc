...
using the optional <<recursive_glob_depth>> setting.
...
===== recursive_glob_depth

Enable expanding `**` into recursive glob patterns. A positive value results in
the rightmost `**` in each path is expanded into a fixed
number of glob patterns. For example: `/foo/**` expands to `/foo`, `/foo/*`,
`/foo/*/*`, and so on. The default is 0, i.e., the feature is disabled.
