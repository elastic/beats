...
Close removed is used to close a harvester directly when a file is removed. Normally a file should only be removed after it already falls under `close_inactive`. In case files are removed early, without this option Filebeat keeps the file open to make sure finishing is completed. In case some files are not completely read because they were removed too early from disk, disable this option.
...
Cleans files from the registry which cannot be found on disk anymore. This does not apply to renamed files or files which were moved to an other directory which is still visible by Filebeat.


In case a shared drive disappears for a short period and appears again, all files will be read again from the beginning as the states were removed. In such cases it is recommended to turn off this option.

This option should only be used in combination `close_removed`.
...
*`nested_parsing`*:: If this setting is enabled, Filebeat recursively traverses JSON to unmarshal all strings encoded as JSON
