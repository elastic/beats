...
When the defined output is blocked and has not confirmed all events, Filebeat keeps resending the events
until the confirmation is persisted to disk in the registrar. Depending on the state of the output,
waiting on the output to acknowledge all events can take seconds or much longer. To ensure that Filebeat is able
to shut down, it does not wait for the confirmation of all events before shutting down. If events are sent, but not `acked`,
their state is not persisted to disk. This can result in Filebeat resending some events upon restart if the events
were sent to the output, but Filebeat was already stopped when the `acked` arrived. 

With `shutdown_timeout`, you can specify the maximum amount of time that Filebeat waits for the complete
sending of all events in the pipeline. Should all events be `acked` before `shutdown_timeout` is reached, Filebeat will
shut down. Determining the right value for `shutdown_timeout` depends heavily on the environment in which Filebeat is running
and the current state of the output.

By default this option is disabled, and Filebeat shuts down without waiting for the complete sending of all events.
