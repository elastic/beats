...
The Cisco Umbrella fileset primarily focuses on reading CSV files from a S3 bucket using the filebeat S3 input.
...
This fileset supports all 4 log types which is:
- Proxy
- Cloud Firewall
- IP Logs
- DNS logs

The Cisco Umbrella fileset is dependant on that the original file path structure is to be followed. This structure is documented https://docs.umbrella.com/deployment-umbrella/docs/log-formats-and-versioning[Umbrella Log Formats and Versioning]:
