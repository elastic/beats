...
Optional Salesforce OAuth login host or token endpoint override for JWT authentication. You can provide either a base URL such as `https://login.salesforce.com` or a full endpoint such as `https://login.salesforce.com/services/oauth2/token`. If empty, this defaults to `var.authentication.jwt_bearer_flow.url`.
...
The Salesforce OAuth login host or token endpoint for user-password authentication. You can provide either a base URL such as `https://login.salesforce.com` or a full endpoint such as `https://login.salesforce.com/services/oauth2/token`.
