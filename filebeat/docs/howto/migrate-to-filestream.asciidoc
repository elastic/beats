...
the all inputs from the configuration before starting Filebeat with the new `filestream` inputs after finishing all the steps. Running old `log` inputs and new `filestream` inputs pointed to the same files will lead to data duplication.
