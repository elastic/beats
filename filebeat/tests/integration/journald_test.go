...
func TestJournaldInputDoesNotDuplicateData(t *testing.T) {
	...
	generateJournaldLogs(t, syslogID, 3, 200)
	...
	// Generate more logs
	generateJournaldLogs(t, syslogID, 5, 200)
	// Restart Filebeat
	filebeat.Start()
...
// 	go generateJournaldLogs(t, syslogID, 3, 100)
...
// 	go generateJournaldLogs(t, syslogID, 5, 100)
// 	filebeat.WaitForLogs("journalctl started with PID", 10*time.Second, "journalctl did not start")
