...
func New(log *logp.Logger, config *Config, nf inputsource.NetworkFunc) (Server, error) {
	...
	case StreamSocket:
		...
		server := &streamServer{config: config, logger: log}
	...
	case DatagramSocket:
		server := &datagramServer{config: config, logger: log}
...
func (s *streamServer) createServer() (net.Listener, error) {
	...
	if err := setSocketOwnership(s.config.Path, s.config.Group, s.logger); err != nil {
		return nil, err
	}
