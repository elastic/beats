...
func TestFilestreamTruncateBlockedOutput(t *testing.T) {
	...
	env.waitUntilEventCount(3)
	env.waitUntilOffsetInRegistry(testlogName, id, len(truncatedTestLines))
...
func TestFilestreamTruncate(t *testing.T) {
	...
	env.waitUntilOffsetInRegistry(testlogName, id, len(moreLines))
...
func TestFilestreamHarvestAllFilesWhenHarvesterLimitExceeded(t *testing.T) {
	const id = "TestFilestreamHarvestAllFilesWhenHarvesterLimitExceeded"
	env := newInputTestingEnvironment(t)
