...
func TestFilestreamDeleteFileReturnsError(t *testing.T) {
	...
	for name, tc := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			...
			// The cursor offset equals the file size, so the first slice hits
			// EOF immediately and (deleter enabled) attempts to delete the file.
			_, err = sess.ReadSlice(v2Ctx, nil)
			if !errors.Is(err, tc.expectedErr) {
				t.Fatalf(
					"ReadSlice did not return the error generated by deleteFile. "+
						"Expecting '%v', got '%v'",
					tc.expectedErr,
					err)
			}
