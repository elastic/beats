...
func (h *Harvester) newLogFileReader() (reader.Reader, error) {
	...
	r, err = encode.NewEncode(h.log, h.encoding, h.config.BufferSize)
	if err != nil {
		return nil, err
	}
	...
	if h.config.DockerJSON != nil {
		...
		r = docker_json.NewDockerJSON(r, h.config.DockerJSON.Stream, h.config.DockerJSON.Partial)
	}

	if h.config.JSON != nil {
		r = json.NewJSON(r, h.config.JSON)
	}
