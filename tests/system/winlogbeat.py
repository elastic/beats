...
class TestCase(unittest.TestCase):
    ...
    def render_config_template(self, template="winlogbeat.yml.j2",
                               output="winlogbeat.yml", **kargs):
        template = self.template_env.get_template(template)
        kargs["winlogbeat"] = self
        output_str = template.render(**kargs)
        with open(os.path.join(self.working_dir, output), "wb") as f:
            f.write(output_str)

    def read_output(self, output_file="output/winlogbeat"):
        jsons = []
    ...
    def log_contains(self, msg, logfile="winlogbeat.log"):
        """
