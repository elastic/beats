...
# Project Structure
- The repo contains several "Beats", which are compiled as standalone binaries
...
- When you're done, run `mage fmt` to ensure all changes are correctly formatted and the imports are correct
...
- Use testify for assertions. Prefer `assert.*`,`require.*` is for fatal errors ONLY
