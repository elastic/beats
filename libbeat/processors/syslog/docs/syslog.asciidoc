...
The syslog processor parses RFC 3164 and/or RFC 5424 formatted syslog messages
...
`drop_failed`:: (Optional) When set, if messages contain errors which cause them to be unparsable,
the processor will drop the message completely. Defaults to `false`.
