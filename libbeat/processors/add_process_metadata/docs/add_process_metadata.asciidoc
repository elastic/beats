...
`cgroup_regex`:: (Optional) A regular expression that matches most of the 
cgroup paths is added by default. This looks like:
`([0-9a-f]{64})(\\.scope)?$` which matches container id for majority of cgroups.
This can be overwritten by specifying regular expression with capture group 
for capturing container id from cgroup path. Examples:
