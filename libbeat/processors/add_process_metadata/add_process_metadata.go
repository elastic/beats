...
func (p *addProcessMetadata) enrich(event common.MapStr, pidField string) (result common.MapStr, err error) {
	...
	if err != nil || metaPtr == nil {
		// no process metadata, lets still try for contianer id
	...
	if b, _ := meta.HasKey("container"); b && len(meta) == 1 {
		for key := range mappings {
			if !strings.Contains(key, "container.id") {
				mappings.Delete(key)
			}
		}
	}
	...
	result = event.Clone()
	for dest, sourceIf := range mappings {
