...
1. DNS SRV lookups for `_ldaps._tcp` (preferred) and `_ldap._tcp`. The processor
  ...
  * If no domain is available, bare queries like `_ldap._tcp` which rely on the
    system's DNS search suffix configuration
+
...
+
Results are ordered by SRV priority and weighted randomly as per RFC 2782 to
