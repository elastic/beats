...
metadata based on which Kubernetes pod the event originated from. By default,
this processor only adds metadata to events that do not already have a
`kubernetes` field. Set `append_fields: true` to merge metadata into events
that already carry a `kubernetes` field without overwriting existing keys.
...
`append_fields`:: (Optional) Default false. When set to true, the processor
will enrich events that already have a `kubernetes` field by merging the
cached metadata into the existing object. Existing keys are preserved; only
fields that are absent from the event are added. Has no effect when the event
does not have a `kubernetes` field.
