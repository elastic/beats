...
`wait_for_metadata` :: (false) When `true`, startup will be blocked until processor is initialized. Startup will fail if processor is unable to connect to Kubernetes API within `wait_for_metadata_timeout` and the process will exit.
If `false`, processor initialization will occur asynchronously. 
`wait_for_metadata_timeout` :: (30s) The maximum time the processor should spend trying to fetch metadata before giving up. Applies regardless of the value of `wait_for_metadata`. To attempt connecting to Kubernetes API indefinitely, set this option to 0. 
