...
`wait_for_processor_ready` :: (false) This specifies if events should be blocked from processing until the processsor is ready.
`wait_for_processor_ready_timeout` :: (10s) This specifies the maximum time the processor will block the pipeline until it is ready. 
