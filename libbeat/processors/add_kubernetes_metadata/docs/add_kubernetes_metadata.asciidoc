...
`wait_for_processor_ready` :: (false) If `true`, processor initialization is performed synchronously and event processing waits until initialization completes. If Kubernetes cannot be reached within `wait_for_processor_ready_timeout`, the processor is initialized as unavailable and events pass through without Kubernetes metadata enrichment. If `false`, initialization runs asynchronously and some early events can be processed without Kubernetes metadata enrichment.
