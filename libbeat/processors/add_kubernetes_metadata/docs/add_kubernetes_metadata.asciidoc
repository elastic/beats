...
`wait_for_metadata` :: (false) If `true` startup will be blocked until processor is initialized. Startup will fail if processor is unable to connect to Kubernetes API within `wait_for_metadata_timeout`.
