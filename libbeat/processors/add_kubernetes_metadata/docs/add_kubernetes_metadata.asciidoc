...
In addition, the node and namespace metadata is added to the pod metadata.
...
value of the field `metricset.host`. When one of this lookup keys matches with one
...
while annotations are not added by default. These settings are useful when labels' and annotations'
