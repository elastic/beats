...
func New(cfg *common.Config) (processors.Processor, error) {
	...
	if err != nil {
		if kubernetes.IsInCluster(config.KubeConfig) {
			...
			logp.Debug("kubernetes", "%v: could not create kubernetes client using config: %v", "add_kubernetes_metadata: %v", os.Getenv("KUBECONFIG"), err)
		} else {
			logp.Debug("kubernetes", "%v: could not create kubernetes client using config: %v", "add_kubernetes_metadatai: %v", config.KubeConfig, err)
		}
