...
func TestEventWithReplaceFieldsFalse(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.title, func(t *testing.T) {
			...
			assert.NoError(t, err)
			assert.Equal(t, c.hostLengthLargerThanOne, len(v.(mapstr.M)) > 1) //nolint:errcheck
			assert.Equal(t, c.hostLengthEqualsToOne, len(v.(mapstr.M)) == 1) //nolint:errcheck
			if c.expectedHostFieldLength != -1 {
				assert.Equal(t, c.expectedHostFieldLength, len(v.(mapstr.M))) //nolint:errcheck
			}
		})
	}
}
