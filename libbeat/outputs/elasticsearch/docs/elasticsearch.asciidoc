...
Notes about the previous example and client based PKI authentication:
  ...
  - `ssl.certificate` and `ssl.key` are ONLY needed if Elasticsearch is configured to require client based PKI authentication (with `xpack.security.http.ssl.client_authentication: required` or `xpack.security.http.ssl.client_authentication: optional`).
  - `ssl.certificate_authorities` need to include the CA used to sign the remote server certificate, not this client cert.
  - If client PKI is used, the remote server (Elasticsearch) should include the CA used for signing the client cert in the list of `xpack.security.http.ssl.certificate_authorities: []` list.
