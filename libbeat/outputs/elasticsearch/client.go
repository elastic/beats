	...
	for i := 0; i < count; i++ {
		...
		if status < 500 {
			if status == http.StatusTooManyRequests {
				...
				if result {
					stats.nonIndexable++
					client.log.Errorf("Can't deliver to dead letter index event (status=%v)", status)
					...
					client.log.Warnf("Cannot index event (status=%v), trying dead letter index", status)
