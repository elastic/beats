...
func TestPublish(t *testing.T) {
	...
	t.Run("sends telemetry headers", func(t *testing.T) {
		var requestCount, contentLength, uncompressedLength, eventCount atomic.Int64

		esMock := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			requestCount.Add(1)

			cl := r.Header.Get(eslegclient.HeaderContentLength)
			if cl != "" {
				l, _ := strconv.Atoi(cl)
				uncompressedLength.Store(int64(l))
			}
