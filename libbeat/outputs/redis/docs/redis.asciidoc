...
Each Redis server can be defined as a `IP:PORT` pair or as a `URL`. For
example: `redis://localhost:6379`,or `rediss://localhost:6379`.
Using URLs one can add a server specific password like as well. For example:
`redis://:password@localhost:6379`.
The `redis` scheme will disable the `ssl` settings for the host, while `rediss`
will enforce TLS. In the later case the output will use the system certificate store if no `ssl`
