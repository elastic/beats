...
Events can be collected into batches. When using the memory queue the maximum batch size is the
value of `queue.mem.flush.min_events`. {beatname_uc} will split batches read from the queue which
are larger than `bulk_max_size` into multiple batches.
