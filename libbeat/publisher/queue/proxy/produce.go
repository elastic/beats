...
func (p *producer) makePushRequest(event queue.Event, canBlock bool) *pushRequest {
	req := &pushRequest{
		event:        event,
...
func (p *producer) Publish(event queue.Event) (queue.EntryID, bool) {
	if p.cancelled {
		return 0, false
	}
...
func (p *producer) TryPublish(event queue.Event) (queue.EntryID, bool) {
	if p.cancelled {
		return 0, false
	}
