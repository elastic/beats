...
func (p *producer) makePushRequest(event queue.Entry, canBlock bool) *pushRequest {
	req := &pushRequest{
		event:        event,
...
func (p *producer) Publish(event queue.Entry) (queue.EntryID, bool) {
	if p.cancelled {
		return 0, false
	}
...
func (p *producer) TryPublish(event queue.Entry) (queue.EntryID, bool) {
	if p.cancelled {
		return 0, false
	}
