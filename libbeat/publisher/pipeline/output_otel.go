...
type otelOutputController struct {
	...
	// queueMode controls how events are delivered to the output.
	queueMode QueueMode

	// directClient is the output client used when queueMode is DirectQueue.
	// Each directProducer calls this client inline.
	directClient outputs.Client

	// queue, consumer, and workers are used only when queueMode is DefaultQueue.
...
func newOTelOutputController(
	...
	switch queueMode {
	case DirectQueue:
		monitors.Logger.Info("Direct output mode enabled: bypassing publisher queue")
		if len(out.Clients) == 0 {
			return nil, fmt.Errorf("no output clients available for direct output mode")
		}
...
func (c *otelOutputController) waitClose(ctx context.Context, _ bool) error {
	switch c.queueMode {
	case DirectQueue:
		c.logger.Infof("Output shutdown started (direct mode).")
		return c.directClient.Close()

	default: // DefaultQueue
		// Signal the queue that we're shutting down, and allow it to drain
...
func (c *otelOutputController) queueProducer(config queue.ProducerConfig) queue.Producer[publisher.Event] {
	switch c.queueMode {
	case DirectQueue:
		return newDirectProducer(c.logger, c.directClient, config.ACK)
	default:
