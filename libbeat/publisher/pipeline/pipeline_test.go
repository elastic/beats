...
type testProducer struct {
	publish func(try bool, event queue.Event) (queue.EntryID, bool)
	cancel  func() int
}
...
func (p *testProducer) Publish(event queue.Event) (queue.EntryID, bool) {
	if p.publish != nil {
...
func (p *testProducer) TryPublish(event queue.Event) (queue.EntryID, bool) {
	if p.publish != nil {
...
func blockingProducer(_ queue.ProducerConfig) queue.Producer {
	...
	return &testProducer{
		publish: func(_ bool, _ queue.Event) (queue.EntryID, bool) {
			waiting.Inc()
			<-sig
			return 0, false
		},
