...
type Map struct {
	mutex sync.RWMutex
	meta  map[uint64]mapstr.Pointer
}

// NewMap instantiates and returns a new meta.Map
func NewMap() *Map {
	return &Map{
		meta: make(map[uint64]mapstr.Pointer),
	}
}

// Store inserts or updates given meta under the given id. Then it returns a MapStrPointer to it
func (m *Map) Store(id uint64, meta mapstr.M) mapstr.Pointer {
	m.mutex.Lock()
	defer m.mutex.Unlock()

	if meta == nil {
		return mapstr.Pointer{}
	}

	p, ok := m.meta[id]
	if !ok {
		// create
		p = mapstr.NewPointer(meta)
		m.meta[id] = p
	} else {
		// update
		p.Set(meta)
