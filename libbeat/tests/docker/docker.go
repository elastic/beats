...
func (c Client) imagePull(imageName string) (err error) {
	ctx := context.Background()
	...
	for retry := 0; retry < 3; retry++ {
		err = func() error {
			respBody, err := c.cli.ImagePull(ctx, imageName, image.PullOptions{})
			if err != nil {
				return fmt.Errorf("pullling image %s: %w", imageName, err)
			}
			...
			if err != nil {
				return fmt.Errorf("reading response for image %s: %w", imageName, err)
			}
			return nil
		}()
		if err == nil {
