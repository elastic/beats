...
ifndef::apm-server[]
*`setup.template.type`*:: Deprecated in 7.16. The type of template to use. The default value is `index`, that loads index templates.
Further options: `legacy`, index templates before Elasticsearch v7.8. Use this to avoid breaking existing deployments.
And `component`, selecting it loads a component template which can be included in new index templates.
