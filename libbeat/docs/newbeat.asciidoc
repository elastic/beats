...
an example Beat called Testbeat. The Beat generator creates all the files required
...
The Beat generator creates a directory called `testbeat` inside of your project folder. 
...
[source,shell]
---------
cd $GOPATH/src/github.com/{user}/testbeat
make setup
---------

The Beat now contains the basic config file, `testbeat.yml`, and template files. The Beat is "complete" in the sense
...
To compile the Beat, make sure you are in the Beat directory (`$GOPATH/src/github.com/{user}/testbeat`) and run:
...
Running this command creates the binary called `testbeat` in `$GOPATH/src/github.com/{user}/testbeat`.
...
The command automatically loads the default config file, `testbeat.yml`, and sends debug output to the console.
...
<1> `done`: Channel used by the `Run()` function to stop when the `Stop()` function is called.
<2> `config`: Configuration options for the Beat
<3> `client`: Publisher that takes care of sending the events to the
  defined output.

The `Beat` parameter received by the `Run` function contains data about the
...
`beater/testbeat.go`). You can modify these implementations, as required, for your Beat.
...
`beater/testbeat.go` when you run the Beat generator:
...
This mirrors the config options that are defined in the config file, `testbeat.yml`. 
