...
If enabled logstash output will send another bulk of events to logstash while
waiting for ACK from last bulk. This option can reduce network latencies and
improve throughput, but will increase memory usage and bandwidth usage in case
of failures. The default is false.
