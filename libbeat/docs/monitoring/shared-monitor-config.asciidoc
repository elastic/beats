...
Refer to <<monitoring-internal-collection>> for usage examples.

==== `monitoring.enabled`

The `monitoring.enabled` config is a boolean setting that controls whether {monitoring} 
is enabled. If set to `true`, monitoring is enabled.
