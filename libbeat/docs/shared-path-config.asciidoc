...
This can either be set in the config, or with the `--system.hostfs` CLI flag.
In Metricbeat, this is used by the system module to read files from `/proc` and `/sys`.
In other beats as well as Metricbeat, this is used for cgroup self-monitoring.
