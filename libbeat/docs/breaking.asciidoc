...
not escaped anymore by default when publishing events.
The old behavior of escaping HTML can still be configured by `escape_html:
true` in the outputs configuration.

//tag::notable-breaking-changes[]
==== Filebeat registry

Filebeat will store the registry in a sub-directory, starting with version 7.0.
The directory is configured using the new `filebeat.registry.path` setting. 
Filebeat will automatically migrate the registry file, if it find the old
registry file at the configured location.
...
release moved most ILM setting from the `output.elasticsearch.ilm` namespace to
