...
all references to old fields, set `migration.6_to_7.enabled: false` so that
field aliases will not be created during your next minor upgrade.
...
`default_field` setting. For example:

// REVIEWERS: Can we provide users with a more realistic example here that
// maybe shows some fields for metricbeat? This one is too generic. 
