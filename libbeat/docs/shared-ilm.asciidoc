...
// REVIEWERS: should we mention that this requires a basic license? I started to
// add a note and then realized that most users will already have a basic license
// so the note might be confusing.
...
used for the rollover index is `%7Bnow%2Fd%7D-000001`. You can change the
...
// REVIEWERS: I had an example of the REST call here, but decided that users my
// cut/paste without reading carefully, so I removed it.
--
