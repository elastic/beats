...
Below an example configuration that decodes the `message` field as CEF after
renaming it to `log.original`. It is best to rename `message` to `log.original`
because the decoded CEF data may contain its own `message` field.
