...
default the fields that you specifiy will be grouped under the `fields`
sub-dictionary in the event.  The `target` setting can be used to change the
sub-dictionary used for grouping the new fields under.  To store the fields as
top-level fields, set `target: ''`. 

``target``:: (Optional) Sub-dictionary to put all fields in to. Defaults to `fields`.
...
under the ECS compliant `labels` sub-dictionary.
...
- add_labels:
    labels:
      number: 1
      label.with.dots: test
      nested:
        label.with.dots: nested
      array:
        - do
	- re
	- with.field: mi
...
The `add_tags` processor adds tags to an list of tags. The tags will be
