...
You can define processors at the top-level in the config to filter all data
collected by {beatname_uc}. Or you can define processors under a specific input
to filter the data collected by that input. For example:
...
This is useful when you have more than one input defined, but want to filter
only the events collected by a specific input.
endif::[]
