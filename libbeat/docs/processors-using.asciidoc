 ...
 * <<add-lxc-metadata,`add_lxc_metadata`>>
...
The `add_lxc_metadata` processor annotates each event with LXC container ID.
...
`match_pids`:: (Optional) A list of fields that contain process IDs.
If the process is running in LXC container then the event will be enriched.
The default value is `["process.pid", "process.ppid"]`.
