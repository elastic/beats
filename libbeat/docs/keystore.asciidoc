...
//TODO: Test the syntax described above
...
// REVIEWERS: Is the above statement true? I'm testing this on MacOS, so it's
// not clear.
...
To pass the value through stdin, use the `--stdin` flag:

["source","sh",subs="attributes"]
----------------------------------------------------------------
cat /file/containing/setting/value | {beatname_lc} keystore add ES_PWD --stdin
----------------------------------------------------------------
