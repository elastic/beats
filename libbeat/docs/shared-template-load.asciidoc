...
//REVIEWERS: Is the following step still necessary?
...
//REVIEWERS: How do you do this when you are using Winlogbeat and starting as a service? Do you need to configure username/password in the config file? I haven't mentioned the config options in the GS because I think it's better for users not to store the passwords in config files. Is that a correct assumption?
