...
==== `exit_on_cert_change.enabled`
If set to `true` {beatname_uc} will exit if any file listed by `key`,
`certificate`, or `certificate_authorities` is modified.

NOTE: This feature is intended to be used when {beatname_uc} is running
under a supervisor that can restart it. The default value is `false`.

[float]
[[exit_on_cert_change_period]]
==== `exit_on_cert_change.period`
...
unnecessary overhead. The default value is 1m.
