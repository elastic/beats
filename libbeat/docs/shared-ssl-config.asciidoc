...
[float]
[[certificate_reload_enabled]]
==== `certificate_reload.enabled`
If set to `true`, {beatname_uc} automatically reloads TLS certificates,
keys, and CA certificates from disk whenever they change, without requiring
a restart. Certificate reload is checked on each TLS handshake, at most once
per `certificate_reload.reload_interval`. The default value is `false`.

[float]
[[certificate_reload_reload_interval]]
==== `certificate_reload.reload_interval`
How often {beatname_uc} checks whether the certificate, key, or CA files
have changed on disk. The default value is `5s`.

[float]
[[exit_on_cert_change_enabled]]
==== `restart_on_cert_change.enabled`

deprecated::[8.19.0,"Use <<certificate_reload.enabled,`certificate_reload.enabled`>> instead. Certificates, keys, and CA certificates are now automatically reloaded on each TLS handshake without requiring a process restart."]
...
deprecated::[8.19.0,"Use <<certificate_reload_reload_interval,`certificate_reload.enabled`>> instead."]
