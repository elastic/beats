...
Use the `aws_elb` autodiscover provider to automatically configure beats against ELB endpoints.

The aws_elb autodiscover provider actually renders once per load balancer listener. Since ELBs
