...
func TestBulkWithResponseFiltering(t *testing.T) {
	// Prepare the test bulk request.
	index := "what"
	...
	tests := map[string]struct {
		responseFiltering bool
		preconfigured     map[string]string
		reqParams         map[string]string
		expected          map[string]string
	}{
		"Bulk with ResponseFiltering": {
		...
		"Bulk with parameters and ResponseFiltering": {
			responseFiltering: true,
			preconfigured: map[string]string{
				"hello": "world",
			},
		...
		"Bulk disabled ResponseFiltering should not append it": {
		...
		"Bulk with only ResponseFiltering": {
			responseFiltering: true,
			preconfigured:     map[string]string{},
			expected: map[string]string{
				"filter_path": "errors,items.*.error,items.*.status",
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			client, _ := NewConnection(ConnectionSettings{
				Parameters:            test.preconfigured,
				URL:                   "http://localhost",
				BulkResponseFiltering: test.responseFiltering,
			})
