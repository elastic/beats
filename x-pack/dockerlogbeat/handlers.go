...
func startLoggingHandler(pm *pipelinemanager.PipelineManager) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			http.Error(w, fmt.Sprintf("error decoding json request: %s", err), http.StatusBadRequest)
			return
		}
		...
		if err != nil {
			http.Error(w, fmt.Sprintf("error creating client config: %s", err), http.StatusBadRequest)
			return
		}
		pm.Logger.Debugf("Got config: %#v", cfg)
		cl, err := pm.CreateClientWithConfig(cfg, startReq.Info, startReq.File)
		if err != nil {
			http.Error(w, fmt.Sprintf("error creating client: %s", err), http.StatusBadRequest)
			return
		}
...
func readLogHandler(pm *pipelinemanager.PipelineManager) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			http.Error(w, fmt.Sprintf("error creating log reader: %s", err), http.StatusBadRequest)
			return
		}
		defer stream.Close()
