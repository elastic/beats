...
//TODO: Remove this when docs are building.
[[elastic-agent-install-service-macos]]

//REVIEWERS: Should we cover deb/rpm here?
...
. Run the `install` command to install {agent} as a managed service. This
command also starts the service.
+
--
