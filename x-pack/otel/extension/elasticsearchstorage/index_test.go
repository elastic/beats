...
func TestIndexCreateBody_ServerlessOmitsShardSettings(t *testing.T) {
	// On serverless, number_of_shards/number_of_replicas are not permitted
	// index settings, so the create body must omit the settings block
	// entirely (even when configured) while still carrying the field mappings.
	serverless := indexCreateBody(true, IndexConfig{NumberOfShards: 5, NumberOfReplicas: 3})
	_, hasSettings := serverless["settings"]
	assert.False(t, hasSettings, "serverless create body must not include settings")
	mappings, ok := serverless["mappings"].(map[string]any)
	require.True(t, ok, "serverless create body must include mappings")
	props, ok := mappings["properties"].(map[string]any)
	require.True(t, ok)
	assert.Contains(t, props, "v")
	assert.Contains(t, props, "enc")
	assert.Contains(t, props, "updated_at")
}

func TestIndexCreateBody_StatefulShardSettings(t *testing.T) {
	cases := []struct {
		name         string
		in           IndexConfig
		wantShards   int
		...
		{"defaults when unset", IndexConfig{}, 1, 0},
		{"custom shards and replicas", IndexConfig{NumberOfShards: 3, NumberOfReplicas: 2}, 3, 2},
		{"non-positive shards falls back", IndexConfig{NumberOfShards: 0, NumberOfReplicas: 1}, 1, 1},
		{"negative replicas falls back", IndexConfig{NumberOfShards: 2, NumberOfReplicas: -1}, 2, 0},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			body := indexCreateBody(false, tc.in)
			settings, ok := body["settings"].(map[string]any)
			require.True(t, ok, "stateful create body must include settings")
			assert.Equal(t, tc.wantShards, settings["number_of_shards"])
			assert.Equal(t, tc.wantReplicas, settings["number_of_replicas"])
