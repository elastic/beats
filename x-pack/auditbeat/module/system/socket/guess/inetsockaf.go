...
func (g *guessInetSockFamily) Probes() ([]tracing.ProbeDef, error) {
	return []tracing.ProbeDef{
		{
			Probe: tracing.Probe{
				Name:      "inet_sock_af_guess",
				Address:   "inet_release",
				Fetchargs: tracing.MakeMemoryDump("+{{.SOCKET_SOCK}}({{.P1}})", 0, inetSockAfDumpSize),
			},
...
// Prepare is a no-op.
func (g *guessInetSockFamily) Prepare(ctx tracing.GuessContext) error {
	g.ctx = ctx
	var ok bool
