...
func (g *guessTCPSendMsg) Probes() ([]tracing.ProbeDef, error) {
	return []tracing.ProbeDef{
		{
			...
			Decoder: tracing.MakeStructDecoder(func() interface{} { return new(tcpSendMsgArgCountGuess) }),
		},
	}, nil
}

// Prepare creates a TCP client-server.
func (g *guessTCPSendMsg) Prepare(ctx tracing.GuessContext) error {
	g.ctx = ctx
