...
// Config is config settings for auditbeat receiver.  The structure of
// which is the same as the auditbeat.yml configuration file.
type Config struct {
...
func (c *Config) Validate() error {
	...
	if _, prs := c.Beatconfig["auditbeat"]; !prs {
		return fmt.Errorf("configuration key 'auditbeat' is required")
	}
	return nil
}
