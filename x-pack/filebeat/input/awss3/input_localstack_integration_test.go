...
func TestInputRunSQS(t *testing.T) {
	...
	uploadS3TestFiles(t, tfConfig.AWSRegion, tfConfig.BucketName,
		"testdata/test.xml", // Skipped (no match).
	)
	...
	//assert.EqualValues(t, s3Input.metrics.sqsMessagesReceivedTotal.Get(), 1) // S3 could batch notifications.
...
func TestInputRunS3(t *testing.T) {
	...
	fmt.Println(tfConfig.BucketName)
	...
	assert.EqualValues(t, s3Input.metrics.s3ObjectsInflight.Get(), 0)
	assert.EqualValues(t, s3Input.metrics.s3ObjectsRequestedTotal.Get(), 0)
	assert.EqualValues(t, s3Input.metrics.s3ObjectsListedTotal.Get(), 0)
	assert.EqualValues(t, s3Input.metrics.s3ObjectsProcessedTotal.Get(), 0)
	assert.EqualValues(t, s3Input.metrics.s3ObjectsAckedTotal.Get(), 0)
	assert.EqualValues(t, s3Input.metrics.s3EventsCreatedTotal.Get(), 0)
}
...
func TestInputRunSNS(t *testing.T) {
	...
	assert.EqualValues(t, s3Input.metrics.sqsMessagesReceivedTotal.Get(), 0) // S3 could batch notifications.
	assert.EqualValues(t, s3Input.metrics.sqsMessagesInflight.Get(), 0)
	assert.EqualValues(t, s3Input.metrics.sqsMessagesDeletedTotal.Get(), 0)
	assert.EqualValues(t, s3Input.metrics.sqsMessagesReturnedTotal.Get(), 0) // Invalid JSON is returned so that it can eventually be DLQed.
	// assert.EqualValues(t, s3Input.metrics.sqsVisibilityTimeoutExtensionsTotal.Get(), 0)
	// assert.EqualValues(t, s3Input.metrics.sqsMessagesWaiting.Get(), 0)
	// assert.EqualValues(t, s3Input.metrics.s3ObjectsInflight.Get(), 0)
	// assert.EqualValues(t, s3Input.metrics.s3ObjectsRequestedTotal.Get(), 7)
