...
func TestConfig(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			"error on ration between max_number_of_messages and number_of_sqs_consumers",
			queueURL,
			"",
			"",
			mapstr.M{
				...
				"number_of_sqs_consumers": "2",
			},
			"the ratio between max_number_of_messages <1> and number_of_sqs_consumers <2> must be greater than 0",
			nil,
		},
		{
			"error on number_of_sqs_consumers == 0",
			queueURL,
			"",
			"",
			mapstr.M{
				"queue_url":               queueURL,
				"number_of_sqs_consumers": "0",
			},
			"number_of_sqs_consumers <0> must be greater than 0",
			nil,
		},
		{
