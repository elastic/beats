...
func (p *pollingDiscoveryV2) poll(ctx context.Context, pipeline beat.Pipeline) {
	...
	for range p.numWorkers {
		...
		go func() {
			defer wg.Done()
			p.worker(ctx, pipeline, workChan)
		}()
	}

	ids, _, ok := p.listObjects(ctx, workChan)
	wg.Wait()
...
func (p *pollingDiscoveryV2) worker(ctx context.Context, pipeline beat.Pipeline, work <-chan state) {
