...
func TestGenerateOktaJWT(t *testing.T) {
	...
	got, err := generateOktaJWT([]byte(testOktaJWKJSON), cnf)
	if err != nil {
...
func TestGenerateOktaJWTPEM(t *testing.T) {
	...
	got, err := generateOktaJWTPEM(testOktaPEMKey, cnf)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	checkToken(t, got, cnf)
...
func TestOktaTokenSource_Token(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "PEM key",
			oktaJWKPEM: testOktaPEMKey,
		},
		{
			name:    "JWK JSON",
			oktaJWK: []byte(testOktaJWKJSON),
		},
	}
