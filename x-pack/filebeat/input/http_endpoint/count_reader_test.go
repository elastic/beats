...
func TestCountReader(t *testing.T) {
	...
	t.Run("commit_prevents_close_release", func(t *testing.T) {
		data := []byte("hello world")
		var inFlight atomic.Int64
		...
		// commit transfers responsibility for releasing bytes.
		committed := r.commit()
		assert.Equal(t, int64(11), committed)

		// Close should NOT release the committed bytes.
		err = r.Close()
		require.NoError(t, err)
		assert.Equal(t, int64(11), inFlight.Load()) // Still 11, not 0.

		// Caller is responsible for releasing.
		inFlight.Add(-committed)
		assert.Equal(t, int64(0), inFlight.Load())
	})

	t.Run("commit_then_read_more", func(t *testing.T) {
		// This tests the edge case where Commit is called mid-read.
		// After Commit, subsequent reads still track bytes.
		data := []byte("hello world")
		var inFlight atomic.Int64
		...
		_, err = r.Read(buf)
		require.NoError(t, err)
		assert.Equal(t, int64(10), inFlight.Load()) // 5 more added

		// Close releases only the post-commit bytes (5 more).
		err = r.Close()
		require.NoError(t, err)
		assert.Equal(t, int64(5), inFlight.Load()) // Committed 5 still there.

		// Release the committed bytes.
		inFlight.Add(-committed)
		assert.Equal(t, int64(0), inFlight.Load())
	})
}
