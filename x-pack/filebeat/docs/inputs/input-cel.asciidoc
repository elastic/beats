...
Values under `state.secret` are always redacted automatically (see <<secret-state-cel,`secret_state`>>).
See <<cel-state-redact,`redact`>> configuration parameters for settings to exclude other sensitive fields from DEBUG logs.
...
* {mito_docs}/lib#AWS[AWS v4 request signing]
** {mito_docs}/lib#hdr-Sign_AWS_from_env-AWS[Sign AWS from env]
** {mito_docs}/lib#hdr-Sign_AWS_from_shared_credentials-AWS[Sign AWS from shared credentials]
** {mito_docs}/lib#hdr-Sign_AWS_from_static_credentials-AWS[Sign AWS from static credentials]
...
The CEL environment enables the https://pkg.go.dev/github.com/google/cel-go/cel#OptionalTypes[optional types] library using the version defined {mito_docs}/lib#OptionalTypesVersion[here] and the https://pkg.go.dev/github.com/google/cel-go/ext#TwoVarComprehensions[two-variable comprehensions extensions] library using the version defined {mito_docs}/lib#TwoVarComprehensionVersion[here].

Additionally, it supports authentication via Basic Authentication, Digest Authentication or OAuth2, or token authentication.

As described in Mito's {mito_docs}/lib#HTTP[HTTP] documentation, configuration for Basic Authentication or token authentication will only affect direct HEAD, GET and POST method calls, not explicity constructed requests run with `.do_request()`. Configuration for Digest Authentication or OAuth2 will be used for all requests made from CEL.
...
NOTE: Token authentication settings are disabled if either `enabled` is set to `false` or the `auth.token` section is missing.

NOTE: Token authentication settings do not affect requests run with `.do_request()`, as explained in {mito_docs}/lib#HTTP[HTTP].
...
[float]
==== `resource.redirect.sensitive_headers`
...
The path must point to a target in the cel directory in the [Filebeat logs directory](https://www.elastic.co/docs/reference/beats/filebeat/directory-layout).
...
The `state.secret` field is always redacted automatically (see <<secret-state-cel,`secret_state`>>). When `secret_state` is configured and `redact` is not, the missing-redact warning is suppressed.
