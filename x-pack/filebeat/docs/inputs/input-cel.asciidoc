...
----
{
    ...
    "url": <resource address>, <3>
    "status_code": <HTTP request status code if a network request>, <4>
    "header": <HTTP response headers if a network request>, <5>
    "rate_limit": <HTTP rate limit map if required by API>, <6>
    "want_more": false <7>
}
----


<1> The `events` field must be present, but may be empty or null.
...
If `rate_limit` is present it must be a map with numeric fields `rate` and `burst`.
The `rate_limit` field may also have a string `error` field and other fields which will be logged.
If it has an `error` field, the `rate` and `burst` will not be used to set rate limit behavior.
The https://pkg.go.dev/github.com/elastic/mito@v0.0.0-20221025013803-2ab0322317ce/lib#Limit[Limit], and https://pkg.go.dev/github.com/elastic/mito@v0.0.0-20221025013803-2ab0322317ce/lib#OktaRateLimit[Okta Rate Limit policy] and https://pkg.go.dev/github.com/elastic/mito@v0.0.0-20221025013803-2ab0322317ce/lib#DraftRateLimit[Draft Rate Limit policy] documentation show how to construct this field.

The `status_code`, `header` and `rate_limit` values may be omitted if the program is not interacting with an HTTP API end-point and so will not contribute to program control.

<7> The evaluation is repeated with the new state after removing events if the events array is not empty unless the `want_more` field is present; if `want_more` is false, a non-empty events array will not result in a repeat.
