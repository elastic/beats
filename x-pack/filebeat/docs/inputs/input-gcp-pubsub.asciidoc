...
Proxy headers may be configured using the `resource.proxy_headers` field which accepts a set of key/value pairs.
...
Optional Google Cloud Pub/Sub API endpoint to connect to. When specified,
Filebeat connects to this endpoint instead of the default
`pubsub.googleapis.com:443`. This is useful for connecting through Private
Service Connect (PSC), custom Universe Domains, or reverse proxies.
