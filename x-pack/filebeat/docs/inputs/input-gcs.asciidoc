...
| `current_state_objects_total`         | Number of objects currently tracked in the state registry.
| `gcs_objects_requested_total`         | Number of GCS objects downloaded.
| `gcs_objects_published_total`         | Number of GCS objects processed that were published.
| `gcs_objects_listed_total`            | Number of GCS objects returned by list operations.
| `gcs_bytes_processed_total`           | Number of GCS bytes processed.
| `gcs_events_created_total`            | Number of events created from processing GCS data.
| `gcs_failed_jobs_total`               | Number of failed jobs.
| `gcs_expired_failed_jobs_total`       | Number of expired failed jobs that could not be recovered.
...
| `gcs_events_per_object`               | Histogram of events in an individual GCS object.
| `gcs_jobs_scheduled_after_validation` | Number of jobs scheduled after validation.
| `source_lag_time`                     | Histogram of the time between the source (Updated) timestamp and the time the object was read.
|=======
