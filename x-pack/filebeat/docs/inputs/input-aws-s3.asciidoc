...
If a domain is provided, the full endpoint URI will be constructed with the regin name in the standard form of `https://s3.<region>.<domain>` supported by AWS and several 3rd party providers.
...
URL of the AWS SQS queue that messages will be received from. (Required when `bucket_arn` and `bucket_name` are not set).
...
ARN of the AWS S3 bucket that will be polled for list operation. (Required when `queue_url` and `bucket_name` are not set).

[float]
==== `bucket_name`

Name of the S3 bucket that will be polled for list operation. Required for 3rd party S3 compatible services but can also be used for AWS hosted buckets instead of `bucket_arn`. (Required when `queue_url` and `bucket_arn` are not set).
