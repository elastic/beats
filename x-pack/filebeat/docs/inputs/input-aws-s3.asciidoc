...
This parameter is required when `non_aws_bucket_name` is defined.
...
This prefix is prepended to the object key using string concatenation when
backing it up to another (or the same) bucket. For example, with
`backup_to_bucket_prefix: processed/` and an object key `logs/data.json`,
the backup key is `processed/logs/data.json`. No path separator is inserted
automatically. Include a trailing `/` in the prefix if you want the backup
