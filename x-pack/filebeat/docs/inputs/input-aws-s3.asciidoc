...
An optional configuration setting that specifies the key at the top level in the
JSON object. This parameter should only be used when log files in S3 bucket is in
JSON format.
