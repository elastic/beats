...
The `max_workers` parameter is aligned with `batch_size` to ensure an even distribution of workloads across all workers. This configuration enables efficient file processing by optimizing resource allocation. The `batch_size` defines the number of blobs retrieved in a single request, and the `max_workers` value should be configured based on factors such as the total number of files to be processed, available system resources, and network bandwidth.

For instance:

- Setting `max_workers=3` would result in each pagination request fetching `3 blobs`, which are then distributed among `3 workers`.
- Setting `max_workers=100` would fetch `100 blobs` per pagination request, distributed among `100 workers`.
