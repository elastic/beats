...
- `toInt`: converts a string to an integer, returns 0 if it fails.
...
This specifies if headers are forwarded in case of a redirect. Default: `false`.
...
Cursor is a list of key value objects where an arbitrary values are defined. The values are interpreted as value templates and a default template can be set. Cursor state is kept between input restarts and updated once all the events for a request are published.
