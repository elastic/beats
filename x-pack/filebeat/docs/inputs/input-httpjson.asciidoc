...
* If the value specified for `early_limit` is less than `1`, the value is treated as a percentage of the Response provided `Limit`. e.g. specifying `0.9` will mean that Requests will continue until reaching 90% of the rate-limit -- for a `limit` value of `120`, the rate-limit starts when the `remaining` reaches `12`.
