...
Use the `httpjson` input to read messages from an HTTP API with JSON or ndjson payloads.  In the case of ndjson each newline separated object becomes an event.
