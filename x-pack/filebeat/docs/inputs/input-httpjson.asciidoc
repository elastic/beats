...
The `replace_with: "fixed_pattern,value"` clause is used to replace a fixed pattern string defined in `request.url` with a given value. It is 
called a fixed pattern string because it always needs to start with a `$.` character, for example: `$.xyz`. The `value` can be hard coded or 
can be extracted from context variables like [`.last_response.*`, `.first_response.*`] etc. The `replace_with` clause can be used in 
combination with the `replace` clause thus providing a lot of flexibility in the logic of chain requests.
