...
NOTE: In Filebeat versions prior to 8.19.18, `max_body_bytes` was only enforced for HMAC-authenticated requests. The limit now applies to all HTTP endpoint requests regardless of the authentication method.
...
filename and will be replaced with the input instance id. For Example, `http-request-trace-*.ndjson`.
 The path must point to a target in the http_endpoint directory in the [Filebeat logs directory](https://www.elastic.co/docs/reference/beats/filebeat/directory-layout).
