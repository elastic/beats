...
- type: http_endpoint
  ...
  listen_address: 192.168.1.1
  listen_port: 8080
  auth_header: someheadername
  auth_header_secret: secretheadertoken
----
...
The header to check for a specific value specified by `auth_header_secret` . Certain webhooks provide the possibility to include a special header and secret to identify the source.

[float]
==== `auth_header`

The secret stored in the header name specified by `auth_header` . Certain webhooks provide the possibility to include a special header and secret to identify the source.
