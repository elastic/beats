...
// BenchmarkFilebeatOTelThroughputMockESNativeEvents is identical to
// BenchmarkFilebeatOTelThroughputMockES but enables native_events_enabled so
// otelconsumer skips the pcommon conversion and passes beat.Events through the
// pipeline via the in-process cache.
func BenchmarkFilebeatOTelThroughputMockESNativeEvents(b *testing.B) {
	const eventCount = 100_000
