...
func TestLogStatusReporter(t *testing.T) {
	...
	fileSymLink := filepath.Join(tmpDir, "test-sym.link")
	require.NoError(t, os.Symlink(filename, fileSymLink))

	t.Logf("writing output to file %s", outPath)

	require.NoError(t, os.Mkdir(outPath, 0775))
	defer func() {
	 ...
	 */
	inputStreamIrregular := getInputStream(unitOneID, fileSymLink, 1)
		...
		Config: &proto.UnitExpectedConfig{
			Streams: []*proto.Stream{{
				...
				Source: tests.RequireNewStruct(map[string]interface{}{
					...
					"id":                                    "filestream-id",
					"paths":                                 path,
					"prospector.scanner.fingerprint.length": 0,
					"prospector.scanner.symlinks":           true,
				}),
			}},
			Type:     "filestream",
			Id:       "filestream-input-test",
			Name:     "filestream-1",
			Revision: 1,
		},
	}
...
func TestXxx(t *testing.T) {
	s, err := os.Stat("//Users/vihasmakwana/test_device")
	fmt.Println(s.Size(), err, "ok")
}
