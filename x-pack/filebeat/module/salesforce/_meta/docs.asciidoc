...
- The `login` fileset collects Login events from the EventLogFile and Event Monitoring.
- The `logout` fileset collects Logout events from the EventLogFile and Event Monitoring.
- The `apex` fileset collects APEX execution logs from the EventLogFile.
- The `setupaudittrail` fileset collects Audit Trails events generated when admins make configuration changes in the org's Setup area.

[options="header"]
|===
| Fileset | EventLogFile | Real-time (Objects)
