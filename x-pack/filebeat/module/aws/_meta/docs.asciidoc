...
This module supports reading s3 server access logs with `s3access` fileset, ELB access logs with `elb` fileset, VPC
...
- module: aws
  cloudtrail:
    ...
    #var.s3_bucket_objects_expiration: 120h
...
*`var.s3_bucket_objects_expiration`*::

Expiration of the S3 objects in the bucket. If you have an expiration set on the
S3 objects in the bucket you can set it here so that internal status of the object will
be purged after the expiration. Setting this value lower than the expiration time of the S3 objects
provokes ingesting the S3 objects multiple times even if they didn't change.
Not having an expiration on the S3 objects or settings this value to `0s` has an impact on
memory usage of Filebat since every S3 object already ingested will be kept forever in the
state store and loaded in memory when Filebeat is started.
