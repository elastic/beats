...
func createReceiver(ctx context.Context, set receiver.Settings, baseCfg component.Config, consumer consumer.Logs) (receiver.Logs, error) {
	...
	if !ok {
		return nil, fmt.Errorf("could not convert otel config to packetbeat config")
	}
	settings := cmd.PacketbeatSettings(nil)
	...
	return &packetbeatReceiver{BeatReceiver: br}, nil
}
