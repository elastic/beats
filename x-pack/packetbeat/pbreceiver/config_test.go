...
func TestUnmarshal(t *testing.T) {
	t.Run("partial path override preserves defaults", func(t *testing.T) {
		...
		userConf := confmap.NewFromStringMap(map[string]any{
			...
			"packetbeat": map[string]any{"interfaces": map[string]any{}},
		})
		...
		assert.Contains(t, cfg.Beatconfig, "packetbeat")
	})
...
func TestValidate(t *testing.T) {
	tests := map[string]struct {
		...
		"No packetbeat section": {
			c:           &Config{Beatconfig: map[string]interface{}{"other": map[string]interface{}{}}},
			hasError:    true,
			errorString: "configuration key 'packetbeat' is required",
		},
		"Valid config": {
			c:           &Config{Beatconfig: map[string]interface{}{"packetbeat": map[string]interface{}{}}},
			hasError:    false,
			errorString: "",
		},
	}
