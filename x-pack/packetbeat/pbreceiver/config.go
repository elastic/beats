...
// Config is config settings for packetbeat receiver.  The structure of
// which is the same as the packetbeat.yml configuration file.
type Config struct {
...
func (c *Config) Validate() error {
	...
	if _, prs := c.Beatconfig["packetbeat"]; !prs {
		return fmt.Errorf("configuration key 'packetbeat' is required")
	}
	return nil
}
