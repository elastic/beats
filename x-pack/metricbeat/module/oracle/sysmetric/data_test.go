...
func TestMetricSet_transform(t *testing.T) {
	type want struct {
		string
	}
	tests := []struct {
		...
		{in: &extractedData{
			...
			want: want{
				string: `{"metrics":{"begin_time":"2022-04-27T12:18:57+05:30","container_id":0,"end_time":"2022-04-27T12:19:57+05:30","group_id":2,"interval_size_csec":6021,"metric_id":2000,"metric_unit":"% (LogRead - PhyRead)/LogRead","name":"Buffer Cache Hit Ratio","value":100}}`,
			},
		},
	}
	for _, tt := range tests {
		t.Run("Test func transform()", func(t *testing.T) {
			...
			if got[0].MetricSetFields.String() != tt.want.string {
				t.Errorf("MetricSet.transform() = %v, want %v", got[0].MetricSetFields.String(), tt.want.string)
			}
		})
	}
}
