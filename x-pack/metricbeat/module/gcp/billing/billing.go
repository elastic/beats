...
// createTagsMap converts a comma-separated string of key-value pairs into a flattened map.
// It is used to convert the tags string generated by the SQL query to a map.
// The SQL query generates tags string by concatenating key-value pairs from an array of structs (tags).
func createTagsMap(tagsItem bigquery.Value) mapstr.M {
	tagsMap := mapstr.M{}
...
func createEvents(row row, tableName, projectID string) mb.Event {
	...
	tagsMap := createTagsMap(row.Tags)
