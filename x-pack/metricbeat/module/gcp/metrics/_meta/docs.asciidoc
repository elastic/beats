...
The time series are then grouped according to their values for the selected labels. 
When using `reducer` that aggregates data points from multiple time series into a single time series, 
the `groupBy` operation will group the results based on the selected labels and maintain those labels in the resulting data.
