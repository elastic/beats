...
func New(base mb.BaseMetricSet) (mb.MetricSet, error) {
	...
	if m.config.MetadataCache {
		...
		m.metadataCacheRegistry = gcp.NewCacheRegistry(m.Logger(), refreshInterval)
	}
...
func (m *MetricSet) mapToEvents(ctx context.Context, timeSeries []timeSeriesWithAligner, sdc metricsConfig) ([]mb.Event, error) {
	...
	if !m.config.ExcludeLabels {
		if metadataService, err = NewMetadataServiceForConfig(m.config, sdc.ServiceName, m.metadataCacheRegistry); err != nil {
