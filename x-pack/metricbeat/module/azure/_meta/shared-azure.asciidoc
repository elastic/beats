...
`lookback_window`:: Optional duration (default `10m`). On restart, the module backfills metrics from the last completed collection end time up to this window. Set to `0` to disable and preserve the previous behavior of always querying from `now - period`. Overlapping documents are deduplicated by Elasticsearch document ID.
