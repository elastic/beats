...
:   *string* Optional. The token audience for the resource manager API is derived automatically from `resource_manager_endpoint` for the Azure public, US Government, and China clouds, so this option is not needed for those environments. Set it only for non-standard environments, such as Azure Stack or other hybrid cloud models with custom audiences. Ex: [https://management.core.windows.net/](https://management.core.windows.net/) for Azure Public. Note that the metrics batch API (`enable_batch_api`) is only available on clouds with a known metrics endpoint and is rejected at startup for custom environments.
