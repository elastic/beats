...
:   *string* Optional. The token audience for the Resource Manager API. For Azure Public, US Government, and China clouds, the audience is derived automatically from `resource_manager_endpoint`, so you do not need to set this option for those environments. Set it only for non-standard environments such as Azure Stack, custom hybrid clouds, or legacy Azure Germany. Ex: `https://management.microsoftazure.de/` for legacy Azure Germany.
