...
// cannedResponse describes one response the stub transport should replay.
...
// the headers of every request it receives. Once the canned list is exhausted the
...
func (t *stubTransport) Do(req *http.Request) (*http.Response, error) {
	...
	canned := t.responses[len(t.responses)-1]
	if len(t.requests) <= len(t.responses) {
		canned = t.responses[len(t.requests)-1]
	}
...
func doThroughPolicy(t *testing.T, status int, headers map[string]string) *http.Response {
	...
	transport := &stubTransport{responses: []cannedResponse{{status: status, headers: headers}}}
