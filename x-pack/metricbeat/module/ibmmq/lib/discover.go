...
type MonType struct {
	...
	subHobj      map[string]ibmmqi.MQObject
}
...
func discoverClasses(metaPrefix string) error {
	...
	if err == nil {
		...
		for i := 0; i < len(elemList); i++ {
			if elemList[i].Type != ibmmqi.MQCFT_GROUP {
				continue
			}
			group := elemList[i]
			cl := new(MonClass)
			...
			for j := 0; j < len(group.GroupList); j++ {
				...
				case ibmmqi.MQIAMO_MONITOR_CLASS:
					classIndex = int(elem.Int64Value[0])
				case ibmmqi.MQIAMO_MONITOR_FLAGS:
					cl.flags = int(elem.Int64Value[0])
				case ibmmqi.MQCAMO_MONITOR_CLASS:
					cl.Name = elem.String[0]
				case ibmmqi.MQCAMO_MONITOR_DESC:
					cl.Description = elem.String[0]
				case ibmmqi.MQCA_TOPIC_STRING:
...
func discoverTypes(cl *MonClass) error {
	...
	if err == nil {
		...
		for i := 0; i < len(elemList); i++ {
			...
			ty.subHobj = make(map[string]ibmmqi.MQObject)
			...
			for j := 0; j < len(group.GroupList); j++ {
				...
				case ibmmqi.MQIAMO_MONITOR_TYPE:
					typeIndex = int(elem.Int64Value[0])
				case ibmmqi.MQCAMO_MONITOR_TYPE:
...
func discoverElements(ty *MonType) error {
	...
	if err == nil {
		...
		for i := 0; i < len(elemList); i++ {
			...
			if elemList[i].Type == ibmmqi.MQCFT_STRING && elemList[i].Parameter == ibmmqi.MQCA_TOPIC_STRING {
			...
			for j := 0; j < len(group.GroupList); j++ {
				...
				case ibmmqi.MQIAMO_MONITOR_ELEMENT:
					elementIndex = int(e.Int64Value[0])
				case ibmmqi.MQIAMO_MONITOR_DATATYPE:
...
func discoverQueues(monitoredQueues string) error {
	...
	for i := 0; i < len(queues) && err == nil; i++ {
		...
		pcfparm.Parameter = ibmmqi.MQIA_Q_TYPE
		pcfparm.Int64Value = []int64{int64(ibmmqi.MQQT_LOCAL)}
		cfh.ParameterCount++
		buf = append(buf, pcfparm.Bytes()...)
		...
		if err == nil {
			cfh, offset := ibmmqi.ReadPCFHeader(buf)
			if cfh.CompCode != ibmmqi.MQCC_OK {
				return fmt.Errorf("PCF command failed with CC %d RC %d", cfh.CompCode, cfh.Reason)
			} else {
				parmAvail := true
				bytesRead := 0
				for parmAvail && cfh.CompCode != ibmmqi.MQCC_FAILED {
					elem, bytesRead = ibmmqi.ReadPCFParameter(buf[offset:])
					offset += bytesRead
		...
		if err == nil {
			...
			for i := 0; i < len(elemList); i++ {
				...
				case ibmmqi.MQIACF_OBJECT_TYPE:
					...
					_ = ibmmqi.MQItoString("OT", int(elemList[i].Int64Value[0]))
				case ibmmqi.MQIAMO_MONITOR_CLASS:
					classidx = int(elemList[i].Int64Value[0])
				case ibmmqi.MQIAMO_MONITOR_TYPE:
					typeidx = int(elemList[i].Int64Value[0])
				case ibmmqi.MQIAMO64_MONITOR_INTERVAL:
			...
			for key, newValue := range values {
				if elem, ok := Metrics.Classes[classidx].Types[typeidx].Elements[key]; ok {
					...
					if oldValue, ok := elem.Values[objectName]; ok {
						if elem.Datatype == ibmmqi.MQIAMO_MONITOR_DELTA {
...
*/
func ParsePCFResponse(buf []byte) ([]*ibmmqi.PCFParameter, bool) {
	var elem *ibmmqi.PCFParameter
	var elemList []*ibmmqi.PCFParameter
	var bytesRead int
	...
	for i := 0; i < int(cfh.ParameterCount); i++ {
		...
		if elem.Type == ibmmqi.MQCFT_GROUP {
			groupElem := elem
	...
	if cfh.Control == ibmmqi.MQCFC_LAST {
		rc = true
	}
	return elemList, rc
}
...
func formatDescription(elem *MonElement) string {
	...
	case ibmmqi.MQIAMO_MONITOR_PERCENT, ibmmqi.MQIAMO_MONITOR_HUNDREDTHS:
		s = s + "_percentage"
	case ibmmqi.MQIAMO_MONITOR_MB, ibmmqi.MQIAMO_MONITOR_GB:
		s = s + "_bytes"
	case ibmmqi.MQIAMO_MONITOR_MICROSEC:
		s = s + "_seconds"
	default:
...
func Normalise(elem *MonElement, key string, value int64) float64 {
	...
	if elem.Datatype == ibmmqi.MQIAMO_MONITOR_PERCENT ||
		elem.Datatype == ibmmqi.MQIAMO_MONITOR_HUNDREDTHS {
		f = f / 100
	} else if elem.Datatype == ibmmqi.MQIAMO_MONITOR_MB {
		f = f * 1024 * 1024
	} else if elem.Datatype == ibmmqi.MQIAMO_MONITOR_GB {
		f = f * 1024 * 1024 * 1024
	} else if elem.Datatype ==
		ibmmqi.MQIAMO_MONITOR_MICROSEC {
		f = f / 1000000
	}
