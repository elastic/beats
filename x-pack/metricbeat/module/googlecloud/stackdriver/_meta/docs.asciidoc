...
* *metric_types*: A list of metric type strings. Each call of the
...
* *metric_type_prefix*: A single string of metric type prefix. For example,
`instance/cpu` is the prefix for metric type `instance/cpu/usage_time`,
`instance/cpu/utilization` etc. Note: `metric_types` and `metric_type_prefix`
...
- module: googlecloud
  ...
  metrics:
    - aligner: ALIGN_NONE
      service: compute
      metric_type_prefix: "instance/cpu"
----
