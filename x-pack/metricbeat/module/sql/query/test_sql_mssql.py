...
class Test(XPackTest):
    ...
    def test_query_without_fetch_from_all_databases(self):
        ...
        """
        self.run_query_test(fetch_from_all_databases=False,
    ...
    def test_query_with_fetch_from_all_databases(self):
        ...
        """
        self.run_query_test(fetch_from_all_databases=True,
  ...
  sql_response_format: table"""
        ...
        self.wait_until(lambda: self.output_count() >=
                        expected_output_count, max_timeout=60)
        ...
        try:
            ...
            for evt in output:
                ...
                self.assertIn(
                    "query", evt["sql"], "Event is missing 'query' key in 'sql' object")
                self.assertIn(
                    "database_name", evt["sql"]["query"], "Event is missing 'database_name' in query results")

                db_name = evt["sql"]["query"]["database_name"]
                self.assertIn(db_name, expected_databases,
            ...
            if fetch_from_all_databases:
                ...
                self.assertEqual(len(
                    found_databases), 1, "Expected only one database when fetch_from_all_databases is False")
                self.assertIn(list(found_databases)[
                              0], expected_databases, "The single database should be one of the expected databases")
