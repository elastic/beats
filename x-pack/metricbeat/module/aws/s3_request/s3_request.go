...
func (m *MetricSet) Fetch(report mb.ReporterV2) error {
	...
	for _, regionName := range m.MetricSet.RegionsList {
		...
		for _, bucketName := range bucketNames {
			event, err := createS3RequestEvents(metricDataOutputs, regionName, bucketName, m.AccountName)
			if err != nil {
				m.Logger().Error(err.Error())
				event.Error = err
				report.Event(event)
...
func createS3RequestEvents(outputs []cloudwatch.MetricDataResult, regionName string, bucketName string, accountID string) (event mb.Event, err error) {
