...
func (m *MetricSet) Fetch(report mb.ReporterV2) error {
	...
	for _, regionName := range m.MetricSet.RegionsList {
		...
		for _, bucketName := range bucketNames {
			event, err := createCloudWatchEvents(metricDataOutputs, regionName, bucketName, m.AccountName)
			if err != nil {
...
func createCloudWatchEvents(outputs []cloudwatch.MetricDataResult, regionName string, bucketName string, accountID string) (event mb.Event, err error) {
