...
`
	...
	require.NoError(t, metricbeat.Cmd.Wait(), "metricbeat setup failed")
	exitCode := metricbeat.Cmd.ProcessState.ExitCode()
	require.Equalf(t, 0, exitCode,
		"metricbeat setup failed: incorrect exit code: %d", exitCode)
