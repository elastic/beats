...
func TestManager(t *testing.T) {
	...
	t.Run("otel management disabled - key missing", func(t *testing.T) {
		beat, err := NewBeatForReceiver(cmd.FilebeatSettings("filebeat"), cfg, consumertest.NewNop(), "testcomponent", zapcore.NewNopCore(), pipeline.DefaultQueue)
	...
	t.Run("otel management enabled", func(t *testing.T) {
		...
		beat, err := NewBeatForReceiver(cmd.FilebeatSettings("filebeat"), tmpCfg, consumertest.NewNop(), "testcomponent", zapcore.NewNopCore(), pipeline.DefaultQueue)
		assert.NoError(t, err)
		assert.NotNil(t, beat.Manager)
