...
func (p *eventRenderer) render() (RenderedEtwEvent, error) {
	...
	event := RenderedEtwEvent{
		...
		EventMessage:      eventInfo.EventMessage,    // TODO formatting
		ProviderMessage:   eventInfo.ProviderMessage, // TODO formatting
...
func getPropertyLength(eventInfo *cachedEventInfo, propInfo *cachedPropertyInfo, r *EventRecord) (uint32, error) {
	...
	if TdhIntypeBinary == propInfo.InType && TdhOuttypeIpv6 == propInfo.OutType {
	...
	return uint32(propInfo.Length), nil
}
