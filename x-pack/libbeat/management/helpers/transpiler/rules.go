...
func (r *SelectIntoRule) Apply(_ AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to select data into configuration: %w", err)
		}
	}()
	target := &Dict{}
...
func (r *RemoveKeyRule) Apply(_ AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to remove key from configuration: %w", err)
		}
	}()
...
func (r *MakeArrayRule) Apply(_ AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to create Dictionary out of configuration: %w", err)
		}
	}()
...
func (r *CopyToListRule) Apply(_ AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to copy segment into configuration: %w", err)
		}
	}()
...
func (r *CopyAllToListRule) Apply(agentInfo AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to copy all nodes into a list: %w", err)
		}
	}()
...
func (r *FixStreamRule) Apply(_ AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to fix stream section of configuration: %w", err)
		}
	}()
...
func (r *InjectIndexRule) Apply(_ AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to inject index into configuration: %w", err)
		}
	}()
...
func (r *InjectStreamProcessorRule) Apply(_ AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to add stream processor to configuration: %w", err)
		}
	}()
	...
	for _, inputNode := range inputsList.value {
		...
		if inputID != nil {
			...
			if !ok {
				return fmt.Errorf("InjectStreamProcessorRule: input processors is not a list")
			}
		...
		for _, streamNode := range streamsList.value {
			...
			processorsList, ok := processorsNode.Value().(*List)
			if !ok {
				return fmt.Errorf("InjectStreamProcessorRule: processors is not a list")
			}
...
func (r *InjectAgentInfoRule) Apply(agentInfo AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to inject agent information into configuration: %w", err)
		}
	}()
	...
	for _, inputNode := range inputsList.value {
		...
		processorsList, ok := processorsNode.Value().(*List)
		if !ok {
			return fmt.Errorf("InjectAgentInfoRule: processors is not a list")
		}
...
func (r *ExtractListItemRule) Apply(_ AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to extract items from configuration: %w", err)
		}
	}()
...
func (r *RenameRule) Apply(_ AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to rename section of configuration: %w", err)
		}
	}()
...
func (r CopyRule) Apply(_ AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to copy section of configuration: %w", err)
		}
	}()
...
func (r *TranslateRule) Apply(_ AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to translate elements of configuration: %w", err)
		}
	}()
...
func (r *TranslateWithRegexpRule) UnmarshalYAML(unmarshal func(interface{}) error) error {
	...
	if err := unmarshal(&tmp); err != nil {
		return fmt.Errorf("cannot unmarshal into a TranslateWithRegexpRule, %w", err)
	}

	re, err := regexp.Compile(tmp.Re)
	if err != nil {
		fmt.Errorf("invalid regular expression for TranslateWithRegexpRule: %w", err)
	}
...
func (r *TranslateWithRegexpRule) Apply(_ AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to translate elements of configuration using regex: %w", err)
		}
	}()
...
func (r *MapRule) Apply(agentInfo AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to apply multiple rules on configuration: %w", err)
		}
	}()
...
func (r *MapRule) UnmarshalYAML(unmarshal func(interface{}) error) error {
	...
	if err := unmarshal(&tmp); err != nil {
		return fmt.Errorf("cannot unmarshal into a MapRule: %w", err)
	}
...
func (r *FilterRule) Apply(_ AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to filter subset of configuration: %w", err)
		}
	}()
...
func (r *FilterValuesRule) Apply(_ AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to filter section based on values from configuration: %w", err)
		}
	}()
...
func (r *FilterValuesWithRegexpRule) UnmarshalYAML(unmarshal func(interface{}) error) error {
	...
	if err := unmarshal(&tmp); err != nil {
		return fmt.Errorf("cannot unmarshal into a FilterValuesWithRegexpRule: %w", err)
	}

	re, err := regexp.Compile(tmp.Re)
	if err != nil {
		return fmt.Errorf("invalid regular expression for FilterValuesWithRegexpRule: %w", err)
	}
	*r = FilterValuesWithRegexpRule{
...
func (r *FilterValuesWithRegexpRule) Apply(_ AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to filter section of configuration using regex: %w", err)
		}
	}()
...
func (r *InjectQueueRule) Apply(agentInfo AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to inject queue parameters into configuration: %w", err)
		}
	}()
...
func (r *InjectHeadersRule) Apply(agentInfo AgentInfo, ast *AST) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("failed to inject headers into configuration: %w", err)
		}
	}()
	...
	if found {
		...
		if found {
			...
			if !ok {
				return fmt.Errorf("headers not a dictionary")
			}
			...
			if !ok {
				return fmt.Errorf("elasticsearch output is not a dictionary")
			}
