...
func newOutputPayloadFixture(t *testing.T) outputPayloadFixture {
	...
	input := &mockClientUnit{
		...
		expected: client.Expected{
			...
			Config: &proto.UnitExpectedConfig{Id: "input-unit"},
		},
	}
	output := &mockClientUnit{
		id:       "output-unit",
		...
		expected: client.Expected{
			State:  client.UnitStateHealthy,
			Config: &proto.UnitExpectedConfig{Id: "output-unit"},
		},
	}
	...
	return outputPayloadFixture{manager: manager, input: input, output: output}
}
...
// which reuse payloadFor. They must not clear the output payload: a later
// identical snapshot is suppressed and would never re-attach it.
func TestPayloadForKeepsTelemetryAcrossUnitChurn(t *testing.T) {
	...
	assert.Equal(t, telemetry, f.manager.payloadFor(client.UnitTypeOutput),
		"output units must keep telemetry through state transitions")
	assert.Nil(t, f.manager.payloadFor(client.UnitTypeInput),
