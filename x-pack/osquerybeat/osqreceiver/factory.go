...
func createReceiver(ctx context.Context, set receiver.Settings, baseCfg component.Config, consumer consumer.Logs) (receiver.Logs, error) {
	...
	if !ok {
		return nil, fmt.Errorf("could not convert otel config to osquerybeat config")
	}
	settings := instance.Settings{
		Name:            "osquerybeat",
		Processing:      processing.MakeDefaultSupport(true, nil, processing.WithECS, processing.WithHost, processing.WithAgentMeta()),
	...
	br, err := xpInstance.NewBeatReceiver(ctx, b, beater.NewReceiver, set)
	if err != nil {
		return nil, fmt.Errorf("error creating %s: %w", Name, err)
	}
	return &osquerybeatReceiver{BeatReceiver: br}, nil
}
...
// Settings.Home should be the path that contains the osquery binaries.
// The supplied Settings.Data should point to the directory where state
// information will be kept.  Both can be overridden by passing in path
// information in the configuration when the receiver is instantiated.
