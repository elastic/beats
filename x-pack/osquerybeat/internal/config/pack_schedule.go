...
var (
	...
	// default_native_schedule.interval and an enabled rrule_schedule.
	ErrPackConflictingScheduleDefaults = errors.New("pack cannot define both default_native_schedule.interval and rrule_schedule")
	...
	// sets schedule_id or start_date without a positive interval.
	ErrPackNativeScheduleMetadataWithoutInterval = errors.New("default_native_schedule cannot set schedule_id or start_date without a positive interval")
)
...
func ValidatePackScheduleDefaults(pack Pack) error {
	...
	if pack.DefaultNativeSchedule.Interval <= 0 &&
		(pack.DefaultNativeSchedule.ScheduleID != "" || pack.DefaultNativeSchedule.StartDate != "") {
...
func ValidatePackQueriesAfterMerge(pack Pack) error {
	...
	for qname, q := range pack.Queries {
		...
		if hasRRuleDefault && !q.RRuleSchedule.IsEnabled() {
			return fmt.Errorf("%w: query %q must use rrule_schedule (pack sets rrule_schedule default)", ErrPackQueryViolatesPackScheduleDefault, qname)
		}
	}
...
func MergeQueryWithPackScheduleDefaults(pack Pack, q Query) (Query, error) {
	...
	if !queryHasRRule {
		...
		if q.ScheduleID == "" && pack.DefaultNativeSchedule.ScheduleID != "" {
			q.ScheduleID = pack.DefaultNativeSchedule.ScheduleID
		}
	...
	if q.ScheduleID == "" && pack.DefaultScheduleID != "" {
		q.ScheduleID = pack.DefaultScheduleID
	}
