...
func TestApplicationShortcutTable(t *testing.T) {
    testCases := []struct {
        name     string
        filePath string
        table    interfaces.Table
    }{
        {name: "Gen table from cached file", filePath: "..\\..\\testdata\\Amcache.hve", table: &ApplicationShortcutTable{}},
        {name: "Gen table from live system", filePath: "C:\\Windows\\AppCompat\\Programs\\Amcache.hve", table: &ApplicationShortcutTable{}},
    }
    ...
    for _, tc := range testCases {
        t.Run(tc.name, func(t *testing.T) {
            ...
            for _, row := range table {
				log.Println(row)
                for _, column := range ApplicationShortcutColumns() {
