...
// REVIEWERS: will the download path include x-path? In BC2, it's 
// https://staging.elastic.co/6.5.0-92a7b7db/downloads/beats/x-pack/functionbeat/functionbeat-6.5.0-windows-x86_64.zip
...
{beatname_lc}.provider.aws.functions:
  - name: cloudwatch
    ...
    triggers:
      - log_group_name: /aws/lambda/myfunction-cloudwatch_logs
...
. Specify a unique name for the S3 bucket that the functions will be uploaded
to. For example:
+
["source","sh",subs="attributes"]
----
...
* `sqs` to collect data from Amazon Simple Queue Service (SQS)
`triggers`:: The trigger that will cause the function to execute. If `type` is
`cloudwatch_logs` logs, specify a list of log groups. If `type` is `sqs`,
...
["source","sh",subs="attributes"]
----------------------------------------------------------------------
./{beatname_lc} deploy cloudwatch -d "*"
----------------------------------------------------------------------
...
["source","sh",subs="attributes"]
----------------------------------------------------------------------
.{backslash}{beatname_lc}.exe deploy cloudwatch
----------------------------------------------------------------------
