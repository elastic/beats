...
func TestNewWithLocation(t *testing.T) {
	scheduler := Create(123, monitoring.NewRegistry(), tarawaTime(), nil, false, logp.NewNopLogger())
...
func TestSchedulerRun(t *testing.T) {
	...
	s := Create(10, monitoring.NewRegistry(), tarawaTime(), nil, false, logp.NewNopLogger())
	defer s.Stop()
...
func TestScheduler_WaitForRunOnce(t *testing.T) {
	s := Create(10, monitoring.NewRegistry(), tarawaTime(), nil, true, logp.NewNopLogger())
...
func TestSchedTaskLimits(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			s := Create(math.MaxInt64, monitoring.NewRegistry(), tarawaTime(), jobConfigByType, false, logp.NewNopLogger())
			var taskArr []int
			mtx := sync.Mutex{}
			wg := sync.WaitGroup{}
			wg.Add(tt.numJobs)
			for i := 0; i < tt.numJobs; i++ {
				...
				go func(tff TaskFunc) {
					sj := newSchedJob(context.Background(), s, "myid", jobType, tff, logp.NewNopLogger())
					sj.run()
					wg.Done()
				}(tf)
			}
...
func BenchmarkScheduler(b *testing.B) {
	s := Create(0, monitoring.NewRegistry(), tarawaTime(), nil, false, logp.NewNopLogger())
