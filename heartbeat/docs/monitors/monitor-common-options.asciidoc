...
If you do not set this explicitly, the monitor's config will be hashed and a generated value will be used. This value will
...
Optional readable name for this monitor by an individual. This value appears in the <<exported-fields,exported fields>>
as `monitor.name`.
...
For stats on the execution of scheduled tasks, enable the HTTP stats server with `http.enabled: true` in heartbeat.yml. Then, run `curl http://localhost:5066/stats | jq .heartbeat.scheduler` to view the scheduler's stats. Stats are provided for both jobs and tasks. Each time a monitor is scheduled, it is considered to be a single job. DNS lookups and executing network requests are defined as tasks. The stats provided are:

* **jobs.active:** The number of actively running jobs/monitors.
* **jobs.missed_deadline:** The number of jobs that are executed after their scheduled time. This can be caused either by overlong long timeouts from the previous job or high load preventing heartbeat from keeping up with work.
...
These are optional fields that can be used to add additional information to the
output. For example, add fields for filtering log
data. Fields can be scalar values, arrays, dictionaries, or any nested
combination of these. By default, the fields that are specified here will be
