...
//Steffen: I wonder if we need a topic that explores deployment scenarios in more detail (like installing on a sub-network where there's a filewall etc. I don't want to complicate the getting started with this level of details. WDYT?
...
Heartbeat uses monitors to configure ping behavior. You configure each
monitor individually. Heartbeat currently provides monitors for ICMP ping,
TCP ping, and HTTP-based ping (see <<heartbeat-overview>> for more about these
...
(`all`).
<4> The `location` specifies the time zone for the scheduler.
+
...
//Steffen: Will we have Heartbeat dashboards for 5.2?

To make it easier for you to see whether your services are responding, we have
created sample Heartbeat dashboards. The dashboards are provided as
examples. We recommend that you
http://www.elastic.co/guide/en/kibana/current/dashboard.html[customize] them to
meet your needs.

//image:./images/heartbeat.png[Hearbeat dashboard] 

//TODO: Update with screen captures of Heartbeat dashboards
