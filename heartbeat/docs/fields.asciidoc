...
+
--
A quantized bucket of time based on this monitor's schedule. Can be used to correlate across geolocations when combined with monitor ID
...
*`monitor.timespan_length`*::
+
--
Total duration, in seconds, timespan applies to
...
+
--
Timestamp matching the lower bound of the timespan of the first event where the event status changed. Useful for grouping errors.
...
+
--
Total duration, in seconds, this status segment has been active
...
+
--
Up or down across all checks in group
