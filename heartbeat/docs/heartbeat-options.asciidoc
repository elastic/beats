...
The total number of redirections Heartbeat will follow. 
If you set it to 0 (zero), Heartbeat will receive the redirect response and not follow it.

For example, you can check for a 302 response (`check.response.status: 302`) if you set `max_redirects: 0`
