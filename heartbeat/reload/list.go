...
// NewHBRunnerList builds and returns a RunnerList
func NewHBRunnerList(name string, factory cfgfile.RunnerFactory, pipeline beat.PipelineConnector, logger *logp.Logger) *HBRunnerList {
	return &HBRunnerList{
		runners:  map[uint64]cfgfile.Runner{},
		factory:  factory,
		pipeline: pipeline,
...
func (r *HBRunnerList) Runners() []cfgfile.Runner {
	r.mutex.Lock()
...
func (r *HBRunnerList) Reload(configs []*reload.ConfigWithMeta) error {
	r.mutex.Lock()
	defer r.mutex.Unlock()
	...
	updateList := map[uint64]*reload.ConfigWithMeta{}
	...
	for _, config := range configs {
		hash, err := r.HashConfig(config.Config)
		if err != nil {
...
func (r *HBRunnerList) Stop() {
	...
	for hash, runner := range r.copyRunnerList() {
		...
		go func(h uint64, run cfgfile.Runner) {
			defer wg.Done()
...
func (r *HBRunnerList) Has(hash uint64) bool {
	r.mutex.RLock()
...
func createRunner(factory cfgfile.RunnerFactory, pipeline beat.PipelineConnector, cfg *reload.ConfigWithMeta) (cfgfile.Runner, error) {
	...
	c, _ := config.NewConfigFrom(cfg.Config)
	if run, ok := factory.(cfgfile.RunnerFactoryWithStatusReporter); ok && cfg.StatusReporter != nil {
