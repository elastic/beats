...
* Linux - As of now, two kernel backends are supported: `kprobes` and `fsnotify`.
...
The kprobes backend uses kernel tracing features to enable capturing user and process
information associated with file integrity monitoring events. It hooks the same kernel
functionality used in fsnotify, so it supports the same kernel versions.
...
source events. Valid values: `kprobes`, `fsnotify`. Default: `fsnotify`.
