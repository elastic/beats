...
<<auditbeat-module-auditd,auditd module>>.
...
the value to `true` it will watch for changes in all sub-directories.

[float]
=== Event Schema Changes

Most field names were changed in 6.2. This is because of the module renames as
well as a desire to use common field names for similar data types between
