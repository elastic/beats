...
func (e *eProcessor) process(_ context.Context, pe *ProbeEvent) error {
	...
	case pe.MaskMonitor == 1:
		...
		monitorPath, match := e.traverser.GetMonitorPath(pe.FileIno, pe.FileDevMajor, pe.FileDevMinor, pe.FileName)
		if !match {
		...
		entry := e.entryCache.Get(dKey{
			Ino:      pe.FileIno,
			DevMajor: pe.FileDevMajor,
			DevMinor: pe.FileDevMinor,
		})

		parentEntry := e.entryCache.Get(dKey{
			Ino:      pe.ParentIno,
		...
		e.entryCache.Add(entry, parentEntry)
	...
	case pe.MaskMoveFrom == 1:
		...
		if parentEntry == nil || parentEntry.Depth >= 1 && !e.isRecursive {
			...
			e.entryCache.MoveClear(uint64(pe.Meta.TID))
			return nil
		}
		...
		e.entryCache.MoveFrom(uint64(pe.Meta.TID), entry)
	...
	case pe.MaskMoveTo == 1:
		...
		if existingChild := parentEntry.GetChild(pe.FileName); existingChild != nil {
			e.entryCache.Remove(existingChild)
			existingChild.Release()
		}

		moved, err := e.entryCache.MoveTo(uint64(pe.Meta.TID), parentEntry, pe.FileName, func(path string) error {
		...
		e.traverser.WalkAsync(newEntryPath, parentEntry.Depth+1, pe.Meta.TID)
