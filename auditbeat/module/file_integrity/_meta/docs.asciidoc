...
rules.
If recursive is set to true, sub directories can also be excluded here by
specifying them. This is an exclusion example for Windows, where the expression 
should always be forged to be case insensitive:
`- '(?i)\\temp\\'`
