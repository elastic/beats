...
// NewKProbesEventReader creates a new EventProducer backed by kProbes.
func NewKProbesEventReader(c Config) (EventProducer, error) {
	return &reader{
		config:  c,
...
func (r kProbesReader) enqueueEvents(done <-chan struct{}) (events []*Event) {
	for {
		ev := r.nextEvent(done)
		if ev == nil {
			return
